//! Julia Set Demo
//!
//! Renders the Julia set and dynamically animates the complex constant `c`.
...
impl WindowApp for JuliaDemo {
    ...
    fn config(&self) -> WindowHostConfig {
        WindowHostConfig {
            title: "🌟 Nova: Julia Set Demo".to_string(),
    ...
    fn update(&mut self, _ctx: WindowContext<'_>) -> Result<(), Self::Error> {
        ...
        {
            self.time += 0.01;
            // Animate `c` around a fascinating orbit
            self.config.c_re = 0.7885 * self.time.cos();
            self.config.c_im = 0.7885 * self.time.sin();

            // Slowly zoom in and out
            self.config.zoom = 1.0 + 0.3 * (self.time * 0.5).sin();
        }
        Ok(())
    }
...
fn main() {
    ...
    error_table
        ...
        .add_row(vec![
            Cell::new(
                "Try running with:
cargo run --example julia_demo --features nova",
            )
            .fg(Color::Green),
        ]);
...
fn print_banner() {
    ...
    table
        ...
        .add_row(vec![
            ...
            Cell::new("Dynamically morphing Julia set fractal").fg(Color::Green),
        ]);
    ...
    controls
        ...
        .add_row(vec![
            Cell::new("Keyboard"),
            Cell::new("Auto-animated morphing"),
        ]);
    println!("{controls}\n");
}
