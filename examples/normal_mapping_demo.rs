...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut normal_map_pixels = vec![0xFFFF8080; 256 * 256];
    ...
    while window.is_open() {
        ...
        let v3_clip = mvp.transform_point(p3);
        ...
        // View Vectors
        // Note: Model Matrix is Identity, so WorldPos = p0, p1, etc.
        let view0 = (camera_pos - p0).normalize();
        let view1 = (camera_pos - p1).normalize();
        let view2 = (camera_pos - p2).normalize();
        let view3 = (camera_pos - p3).normalize();
        ...
        fill_triangle_normal_mapped(
            &mut framebuffer,
            &mut zbuffer,
            (v0_clip, uv0, nw, tw, view0),
            (v1_clip, uv1, nw, tw, view1),
            (v2_clip, uv2, nw, tw, view2),
            &diffuse_map,
            &normal_map,
            light_dir,
            light_color,
