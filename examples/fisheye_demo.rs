//! Demonstration of the Fisheye Lens Filter.
//!
//! Creates a colorful grid pattern and applies a Fisheye filter.
//!
//! Run with:
//! ```sh
//! cargo run --example fisheye_demo --no-default-features --features "backend-tui parallel nova" --release
//! ```
...
fn print_banner() {
    ...
    table
        ...
        .add_row(vec![
            ...
            Cell::new("Radial lens distortion").fg(Color::Green),
        ])
        .add_row(vec![
            Cell::new("Renderer"),
    ...
    controls
        ...
        .add_row(vec![Cell::new("Keyboard"), Cell::new("Auto-pulsing")]);
    println!("{controls}\n");
}

fn main() {
    print_banner();
    let mut window = Window::new("Fisheye Demo", 800, 600).unwrap();
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            // Draw a red center line to make the distortion obvious
    ...
    while window.is_open() {
        ...
        angle += 0.05;
        // pulse strength between 0.5 and 2.5
        let pulse = (angle.sin() + 1.0) + 0.5;
