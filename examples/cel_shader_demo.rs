...
fn print_banner() {
    ...
    table
        ...
        .add_row(vec![
            ...
            Cell::new("Toon / Cel-Shading effect").fg(Color::Green),
        ])
        .add_row(vec![
            Cell::new("Features"),
            Cell::new("Edge Detection & Color Quantization").fg(Color::Yellow),
        ]);
...
impl WindowApp for CelShaderDemoApp {
    type Error = AppError;

    fn config(&self) -> WindowHostConfig {
        WindowHostConfig {
            title: "Abrash - Cel Shader Demo".to_string(),
            width: WIDTH,
            height: HEIGHT,
            vsync: true,
        }
    ...
    fn render(&mut self, _ctx: WindowContext<'_>) -> Result<(), Self::Error> {
        ...
        apply_cel_shader(&mut self.framebuffer, &config);
...
fn main() -> Result<(), AppError> {
    print_banner();
    run_windowed(CelShaderDemoApp::new().unwrap());
    Ok(())
}
