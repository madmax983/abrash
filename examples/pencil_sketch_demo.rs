use abrash::experimental::pencil_sketch::apply_pencil_sketch;
...
fn print_banner() {
    println!("\n{}", "✨ Pencil Sketch Filter Demo".bold().cyan());
    println!("{}", "=======================".dark_grey());
    ...
    table
        ...
        .add_row(vec![
            ...
            Cell::new("Retro Pencil Sketch Post-Processing Filter").fg(Color::Green),
        ]);
...
impl WindowApp for PencilSketchDemoApp {
    ...
    fn config(&self) -> WindowHostConfig {
        WindowHostConfig {
            title: "Abrash - Pencil Sketch Demo".to_string(),
            width: WIDTH,
            height: HEIGHT,
            vsync: true,
        }
    ...
    fn render(&mut self, _ctx: WindowContext<'_>) -> Result<(), Self::Error> {
        ...
        let blur_radius = ((self.total_time.sin() * 0.5 + 0.5) * 5.0) as u32 + 1;
        apply_pencil_sketch(&mut self.framebuffer, blur_radius);
...
fn main() -> Result<(), AppError> {
    print_banner();
    run_windowed(PencilSketchDemoApp::new().unwrap());
    Ok(())
}
