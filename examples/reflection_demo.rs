...
fn main() {
    ...
    tex_x_pos.pixels.fill(0xFF0000FF); // Red
    let mut tex_x_neg = Texture::new(256, 256).unwrap();
    tex_x_neg.pixels.fill(0xFF00FF00); // Green
    let mut tex_y_pos = Texture::new(256, 256).unwrap();
    tex_y_pos.pixels.fill(0xFFFF0000); // Blue
    let mut tex_y_neg = Texture::new(256, 256).unwrap();
    tex_y_neg.pixels.fill(0xFFFFFF00); // Cyan
    let mut tex_z_pos = Texture::new(256, 256).unwrap();
    tex_z_pos.pixels.fill(0xFFFF00FF); // Magenta
    let mut tex_z_neg = Texture::new(256, 256).unwrap();
    tex_z_neg.pixels.fill(0xFF00FFFF); // Yellow
