//! Abrash Graphics Demo - `RetroFX` (Nova Feature)
//!
//! Demonstrates the post-processing pipeline.
...
#[cfg(feature = "nova")]
use abrash::experimental::retrofx::{
    ChromaticAberration, RetroPipeline, Scanlines,
};
...
fn run_demo() -> Result<(), Box<dyn std::error::Error>> {
    let mut window = Window::new("Abrash - RetroFX (Nova)", WIDTH, HEIGHT)?;
    ...
    // Create RetroFX Pipeline
    let pipeline = RetroPipeline::new()
        // Shift RGB channels for "glitch" look
        .with_effect(ChromaticAberration::new(4))
        // Darken every other line for CRT look
        .with_effect(Scanlines::new(0.3, 2))
        // Optional: Grayscale or Invert
        // .with_effect(Grayscale)
        ;
