...
use abrash_render::experimental::video_feedback::{VideoFeedbackConfig, apply_video_feedback};
...
fn print_banner() {
    println!("\n{}", "📺  Video Feedback Demo".bold().magenta());
    println!("{}", "=======================".dark_grey());
    ...
    table
        ...
        .add_row(vec![
            ...
            Cell::new("Retro temporal feedback effect").fg(Color::Green),
        ])
        .add_row(vec![
            Cell::new("Effect"),
            Cell::new("Infinite trails with rotation and zoom").fg(Color::Yellow),
        ]);
...
impl WindowApp for VideoFeedbackDemoApp {
    ...
    fn config(&self) -> WindowHostConfig {
        WindowHostConfig {
            title: "Abrash - Video Feedback Demo".to_string(),
            width: WIDTH,
            height: HEIGHT,
            vsync: true,
        }
    ...
    fn render(&mut self, _ctx: WindowContext<'_>) -> Result<(), Self::Error> {
        ...
        // Animate cubes moving in a circle to show off trails
        let radius = 5.0;
        let x1 = (self.time).cos() * radius;
        let y1 = (self.time).sin() * radius;

        let x2 = (self.time + PI).cos() * radius;
        let y2 = (self.time + PI).sin() * radius;

        let model1 = Mat4::translation(x1, y1, 0.0)
            * Mat4::rotation_x(self.time)
            * Mat4::rotation_y(self.time);
        let model2 = Mat4::translation(x2, y2, 0.0)
            * Mat4::rotation_x(-self.time)
            * Mat4::rotation_y(-self.time);
        ...
        let config = VideoFeedbackConfig {
            feedback_amount: 0.95, // heavy trails
            rotation: 0.02,        // slight spin
            scale: 0.98,           // zoom inward
            offset_x: 0.0,
            offset_y: 0.0,
        };

        apply_video_feedback(&mut self.framebuffer, &config);
