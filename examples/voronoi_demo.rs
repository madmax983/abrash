...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    print_banner();
    let mut window = Window::new("Abrash - Voronoi Demo", WIDTH, HEIGHT)?;
    ...
    while window.is_open() {
        ...
        render_scene(&mut framebuffer, &mut zbuffer, &cube, angle_y, view_proj);

        // Modulate pixel sort parameters over time
        let phase = (total_time * 0.5).sin() * 0.5 + 0.5;

        // Apply Voronoi Effect
        let config = VoronoiConfig {
            num_seeds: 200,
            use_image_color: true,
            metric: 1.0 + phase, // Varies between Manhattan (1.0) and Euclidean (2.0)
            seed: (total_time * 10.0) as u32, // Animate seeds slightly
            border_thickness: phase * 2.0, // Varies border thickness
            border_color: 0xFF000000,
        };
        apply_voronoi(&mut framebuffer, &config);
