...
fn clear_rect_optimized(fb: &mut Framebuffer, x: i32, y: i32, width: u32, height: u32, color: u32) {
    if width == 0 || height == 0 {
        return;
    }
...
fn clear_rect_chunk_unsafe(fb: &mut Framebuffer, x: i32, y: i32, width: u32, height: u32, color: u32) {
    if width == 0 || height == 0 {
        return;
    }
    ...
    let start_x = x.max(0).min(fb.width() as i32) as u32;
    let start_y = y.max(0).min(fb.height() as i32) as u32;
    let end_x = x2.max(0).min(fb.width() as i32) as u32;
    let end_y = y2.max(0).min(fb.height() as i32) as u32;
    ...
    if sx == 0 && ex == w {
        ...
        for row in fb.as_mut_slice()[start_idx..end_idx].chunks_exact_mut(w) {
            unsafe { row.get_unchecked_mut(sx..ex).fill(color); }
        }
    }
}
...
fn bench_clear_rect_opt(c: &mut Criterion) {
    ...
    group.bench_function("chunk_unsafe_1080p", |b| {
        b.iter(|| {
            clear_rect_chunk_unsafe(
                &mut fb,
                black_box(100),
                black_box(100),
                black_box(1000),
