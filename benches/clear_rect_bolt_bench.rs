...
fn clear_rect_baseline(fb: &mut Framebuffer, x: i32, y: i32, width: u32, height: u32, color: u32) {
    if width == 0 || height == 0 {
        return;
    }
...
fn clear_rect_chunks(fb: &mut Framebuffer, x: i32, y: i32, width: u32, height: u32, color: u32) {
    if width == 0 || height == 0 {
        return;
    }
    ...
    if sx == 0 && ex == w {
        ...
        let slice = &mut fb.as_mut_slice()[start_idx..end_idx];
...
fn bench_clear_rect_bolt(c: &mut Criterion) {
    let mut fb = Framebuffer::new(1920, 1080).unwrap();
    let mut group = c.benchmark_group("clear_rect_bolt");

    group.bench_function("baseline_1080p", |b| {
        b.iter(|| {
            clear_rect_baseline(
                &mut fb,
                black_box(100),
    ...
    group.bench_function("chunks_1080p", |b| {
        b.iter(|| {
            clear_rect_chunks(
                &mut fb,
                black_box(100),
...
criterion_group!(benches, bench_clear_rect_bolt);
