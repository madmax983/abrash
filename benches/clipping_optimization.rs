...
fn lerp_tuple(a: (Vec3, f32), b: (Vec3, f32), t: f32) -> (Vec3, f32) {
...
pub fn clip_triangle_to_frustum_legacy(
    ...
    for plane in planes {
        ...
        let (prev_pos, prev_w) = (prev_v.0, prev_v.1);
        let mut prev_d = plane(prev_pos, prev_w);

        for &curr_v in buf1.iter().take(count) {
            let (curr_pos, curr_w) = (curr_v.0, curr_v.1);
            ...
            if curr_d >= 0.0 {
                if prev_d < 0.0 {
                    ...
                    if out_count < 12 {
                        buf2[out_count] = lerp_tuple(prev_v, curr_v, t);
                        out_count += 1;
                    }
                }
                if out_count < 12 {
...
fn bench_clipping(c: &mut Criterion) {
    ...
    group.bench_function("trivial_accept_legacy", |b| {
        ...
        b.iter(|| clip_triangle_to_frustum_legacy(black_box(v0), black_box(v1), black_box(v2)));
    });
