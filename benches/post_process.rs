    ...
    c.bench_function("apply_ssao 1080p", |b| {
        b.iter(|| {
            post_process::apply_ssao(
                ...
                black_box(&zb),
                black_box(&proj),
                black_box(&post_process::SsaoConfig { radius: 1.0, bias: 0.001, intensity: 2.0 }),
            );
        });
    });
}
    ...
    c.bench_function("apply_depth_of_field 1080p", |b| {
        b.iter(|| {
            post_process::apply_depth_of_field(
                black_box(&mut fb),
                black_box(&zb),
                black_box(&post_process::DepthOfFieldConfig { focus_dist: 0.7, focus_range: 0.1, blur_radius: 5 }),
            );
        });
    });
}
