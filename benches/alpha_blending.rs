...
fn bench_fill_triangle_opaque(c: &mut Criterion) {
    c.bench_function("fill_triangle_opaque", |b| {
        ...
        let c0 = Vec4::new(1.0, 0.0, 0.0, 1.0); // Alpha 1.0
...
fn bench_fill_triangle_transparent(c: &mut Criterion) {
    c.bench_function("fill_triangle_transparent", |b| {
        let mut fb = Framebuffer::new(800, 600).unwrap();
        let mut zb = ZBuffer::new(800, 600).unwrap();
        // Pre-fill framebuffer to force blending
        fb.clear(0xFF808080);
        ...
        let c0 = Vec4::new(1.0, 0.0, 0.0, 0.5); // Alpha 0.5
