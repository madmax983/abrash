...
fn bench_fill_triangle_point_lit(c: &mut Criterion) {
    let width = 1920;
    let height = 1080;
    ...
    // Format: ((ClipPos, W), Normal, WorldPos)

    // World Positions (scaled out to make sense for lighting)
    ...
    c.bench_function("fill_triangle_point_lit", |b| {
        b.iter(|| {
            fb.clear(0);
            zb.clear();
...
criterion_group!(benches, bench_fill_triangle_point_lit);
