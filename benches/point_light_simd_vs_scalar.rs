...
fn bench_fill_triangle_point_lit_simd_vs_scalar(c: &mut Criterion) {
    let mut group = c.benchmark_group("Point Light Rasterization");
    ...
    group.bench_function("Optimized SIMD Point Lit", |b| {
        b.iter(|| {
            fb.clear(0);
            zb.clear();
...
criterion_group!(benches, bench_fill_triangle_point_lit_simd_vs_scalar);
