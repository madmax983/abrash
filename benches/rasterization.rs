...
fn bench_fill_triangle_alpha(c: &mut Criterion) {
    c.bench_function("fill_triangle_alpha", |b| {
        let mut fb = Framebuffer::new(800, 600).unwrap();
        let mut zb = ZBuffer::new(800, 600).unwrap();
        ...
        b.iter(|| {
            zb.clear();
            // Fill background with white to force blending calc
            fb.clear(0xFFFFFFFF);
