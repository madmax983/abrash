...
fn bench_clear_rect_1080p(c: &mut Criterion) {
    ...
    c.bench_function("framebuffer/clear_rect_1080p", |b| {
        b.iter(|| {
            fb.clear_rect(black_box(100), black_box(100), black_box(1000), black_box(500), black_box(0xFFFFFFFF));
        });
    });
}
...
fn bench_clear_rect_4k(c: &mut Criterion) {
    ...
    c.bench_function("framebuffer/clear_rect_4k", |b| {
        b.iter(|| {
            fb.clear_rect(black_box(500), black_box(500), black_box(2000), black_box(1000), black_box(0xFFFFFFFF));
        });
    });
}
