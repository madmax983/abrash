...
fn clear_rect_unsafe(fb: &mut Framebuffer, x: i32, y: i32, width: u32, height: u32, color: u32) {
    if width == 0 || height == 0 { return; }
    let x1 = x;
    let y1 = y;
    let x2_i64 = i64::from(x) + i64::from(width);
    let y2_i64 = i64::from(y) + i64::from(height);
    let x2 = if x2_i64 > i64::from(i32::MAX) { i32::MAX } else { x2_i64 as i32 };
    let y2 = if y2_i64 > i64::from(i32::MAX) { i32::MAX } else { y2_i64 as i32 };
...
fn clear_rect_safe(fb: &mut Framebuffer, x: i32, y: i32, width: u32, height: u32, color: u32) {
    if width == 0 || height == 0 { return; }
    let x1 = x;
    let y1 = y;
    let x2_i64 = i64::from(x) + i64::from(width);
...
fn bench_clear_rect_safe_vs_unsafe(c: &mut Criterion) {
    let mut fb = Framebuffer::new(3840, 2160).unwrap();
    let mut group = c.benchmark_group("clear_rect_optimization");

    group.bench_function("clear_rect_unsafe", |b| {
        b.iter(|| {
            clear_rect_unsafe(
                &mut fb,
                black_box(500),
    ...
    group.bench_function("clear_rect_safe_chunks", |b| {
        b.iter(|| {
            clear_rect_safe(
                &mut fb,
                black_box(500),
...
criterion_group!(benches, bench_clear_rect_safe_vs_unsafe);
