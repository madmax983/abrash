...
fn bench_draw_circle_neg(c: &mut Criterion) {
    let mut fb = Framebuffer::new(800, 600).unwrap();

    c.bench_function("draw_circle_neg_r100", |b| {
        b.iter(|| {
            draw_circle(
                &mut fb,
                black_box(400),
...
fn bench_fill_circle_neg(c: &mut Criterion) {
    let mut fb = Framebuffer::new(800, 600).unwrap();

    c.bench_function("fill_circle_neg_r100", |b| {
        b.iter(|| {
            fill_circle(
                &mut fb,
                black_box(400),
...
criterion_group!(
    ...
    bench_draw_circle,
    bench_fill_circle,
    bench_draw_circle_neg,
    bench_fill_circle_neg
);
