...
impl AbrashBackend {
    ...
    /// By taking ownership of the `Mesh` instead of borrowing (`&Mesh`), we can forward it
    /// directly to `create_mesh_owned`. This prevents an expensive `.clone()` operation and
    /// eliminates a large O(N) heap allocation when initializing the application.
