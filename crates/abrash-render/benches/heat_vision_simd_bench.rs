...
fn bench_heat_vision_scalar(c: &mut Criterion) {
    ...
    // Force non-SIMD tail processing or smaller resolutions to expose the scalar loop overhead
    // We pick odd resolutions to ensure the SIMD loop (which processes in chunks of 8)
    // leaves a significant scalar tail.
    let resolutions = [(323, 241), (799, 601), (1919, 1079)];
