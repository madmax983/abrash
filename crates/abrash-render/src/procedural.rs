...
static PLASMA_LUT: std::sync::OnceLock<[u32; 256]> = std::sync::OnceLock::new();

fn get_plasma_lut() -> &'static [u32; 256] {
    PLASMA_LUT.get_or_init(|| {
        let mut lut = [0u32; 256];
        for i in 0..=255 {
            let normalized = i as f32 / 255.0;
            let (r_sin, _) = abrash_core::math::fast_sin_cos(normalized * std::f32::consts::PI);
            let r = (r_sin.abs() * 255.0) as u32;
            let (g_sin, _) =
                abrash_core::math::fast_sin_cos((normalized * std::f32::consts::PI) + 2.0);
            let g = (g_sin.abs() * 255.0) as u32;
            let (b_sin, _) =
                abrash_core::math::fast_sin_cos((normalized * std::f32::consts::PI) + 4.0);
...
pub fn plasma(width: u32, height: u32) -> Result<Texture, &'static str> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let mut index = (normalized * 255.0) as usize;
            if index > 255 {
                index = 255;
            }
            let color = lut[index];
