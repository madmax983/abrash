...
pub fn plasma(width: u32, height: u32) -> Result<Texture, &'static str> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let dist_sq = u.mul_add(u, v * v);
            let (v4, _) = fast_sin_cos(dist_sq * 0.005);

            let val = (v1 + v2 + v3 + v4) * 0.25; // -1 to 1
            let normalized = (val + 1.0) * 0.5; // 0 to 1

            // Map to a psychedelic palette
            let pi_norm = normalized * std::f32::consts::PI;
            let (r_sin, _) = fast_sin_cos(pi_norm);
            let r = (r_sin.abs() * 255.0) as u32;
            let (g_sin, _) = fast_sin_cos(pi_norm + 2.0);
            let g = (g_sin.abs() * 255.0) as u32;
            let (b_sin, _) = fast_sin_cos(pi_norm + 4.0);
