...
///
/// let mut fb = abrash_core::framebuffer::Framebuffer::new(1, 1).unwrap();
...
///
/// let mut fb = abrash_core::framebuffer::Framebuffer::new(1, 2).unwrap();
    ...
    #[cfg(all(target_arch = "x86_64", feature = "simd"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            unsafe {
                let len = pixels.len();
                let simd_len = len & !7;
                simd::apply_solarize_avx2(&mut pixels[..simd_len], threshold);
                apply_solarize_scalar(&mut pixels[simd_len..], threshold);
            }
            return;
        }
    }
    apply_solarize_scalar(pixels, threshold);
}

fn apply_solarize_scalar(pixels: &mut [u32], threshold: u8) {
...
///
/// let mut fb = abrash_core::framebuffer::Framebuffer::new(100, 100).unwrap();
...
///
/// let mut fb = abrash_core::framebuffer::Framebuffer::new(800, 600).unwrap();
...
mod simd {
    ...
    pub unsafe fn apply_solarize_avx2(pixels: &mut [u32], threshold: u8) {
        ...
        for _ in 0..(simd_len / 8) {
            ...
            // To compare unsigned 8-bit integers, subtract 128 (toggle MSB) and use signed compare
            let p_offset = _mm256_add_epi8(p, _mm256_set1_epi8(128u8 as i8));
...
mod tests {
    ...
    #[test]
    fn test_apply_solarize() {
        let mut fb = Framebuffer::new(3, 1).unwrap();
        // Base pixels
        fb.set_pixel(0, 0, 0xFF80_8080); // Mid Gray (128) -> Above threshold (127), should invert (255 - 128 = 127)
        ...
        assert_eq!((p0 >> 8) & 0xFF, 127, "G channel failed at x=0");
        assert_eq!(p0 & 0xFF, 127, "B channel failed at x=0");

        let p1 = fb.get_pixel(1, 0).unwrap();
        assert_eq!((p1 >> 16) & 0xFF, 64, "R channel failed at x=1");
        assert_eq!((p1 >> 8) & 0xFF, 64, "G channel failed at x=1");
        assert_eq!(p1 & 0xFF, 64, "B channel failed at x=1");

        let p2 = fb.get_pixel(2, 0).unwrap();
        assert_eq!((p2 >> 16) & 0xFF, 63, "R channel failed at x=2");
        assert_eq!((p2 >> 8) & 0xFF, 63, "G channel failed at x=2");
        assert_eq!(p2 & 0xFF, 63, "B channel failed at x=2");
    }
