...
unsafe fn find_min_max_simd(depths: &[f32]) -> (f32, f32, bool) {
    ...
    while i + 8 <= len {
        ...
        if _mm256_movemask_ps(mask) != 0 {
            has_content = true;
            // f32::MAX.min(f32::INFINITY) is natively f32::MAX, so masking is redundant for min
...
unsafe fn apply_heat_vision_simd(pixels: &mut [u32], depths: &[f32], min_z: f32, scale: f32) {
    ...
    while i + 8 <= len {
        ...
        let mask1 = _mm256_cmpgt_epi32(c256, t_clamped);
        let mask2 = _mm256_cmpgt_epi32(c512, t_clamped);
        let mask3 = _mm256_cmpgt_epi32(c768, t_clamped);

        let r_part2 = _mm256_sub_epi32(c511, t_clamped);
        let r_256_511 = _mm256_max_epi32(zero_vec, r_part2);
        let r_0_511 = _mm256_blendv_epi8(r_256_511, c255, mask1);
        let r = _mm256_blendv_epi8(zero_vec, r_0_511, mask2);

        let g_part3 = _mm256_sub_epi32(c1023, t_clamped);
        let g_mid = _mm256_blendv_epi8(c255, t_clamped, mask1);
        let g = _mm256_blendv_epi8(g_part3, g_mid, mask3);

        let b_part2 = _mm256_sub_epi32(t_clamped, c512);
        let b_512_767 = _mm256_max_epi32(zero_vec, b_part2);
        let b_512_1023 = _mm256_blendv_epi8(c255, b_512_767, mask3);
        let b = _mm256_blendv_epi8(b_512_1023, zero_vec, mask2);

        let r_shifted = _mm256_slli_epi32::<16>(r);
        let g_shifted = _mm256_slli_epi32::<8>(g);
        let argb = _mm256_or_si256(
            _mm256_or_si256(alpha, r_shifted),
