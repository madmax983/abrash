...
unsafe fn apply_heat_vision_simd(pixels: &mut [u32], depths: &[f32], min_z: f32, scale: f32) {
    ...
    let c_alpha = _mm256_set1_epi32(0xFF00_0000_u32 as i32);
    ...
    while i + 8 <= len {
        ...
        // Ensure not negative
        let t_clamped_low = _mm256_max_epi32(t_i32, c0);

        // Clamp to 1023
        let t = _mm256_min_epi32(t_clamped_low, max_t_vec);

        // Masks for segment conditions
        let mask1 = _mm256_cmpgt_epi32(c256, t); // 256 > t
        let mask2 = _mm256_cmpgt_epi32(c512, t); // 512 > t
        let mask3 = _mm256_cmpgt_epi32(c768, t); // 768 > t

        // r = mask2 ? (mask1 ? 255 : 511 - t) : 0
        let r_seg1 = _mm256_sub_epi32(c511, t);
        let r_seg01 = _mm256_blendv_epi8(r_seg1, c255, mask1);
        let r = _mm256_blendv_epi8(c0, r_seg01, mask2);

        // g = mask3 ? (mask1 ? t : 255) : 1023 - t
        let g_seg3 = _mm256_sub_epi32(c1023, t);
        let g_mid = _mm256_blendv_epi8(c255, t, mask1);
        let g = _mm256_blendv_epi8(g_seg3, g_mid, mask3);

        // b = mask3 ? (mask2 ? 0 : t - 512) : 255
        let b_seg2 = _mm256_sub_epi32(t, c512);
        let b_mid = _mm256_blendv_epi8(b_seg2, c0, mask2);
        let b = _mm256_blendv_epi8(c255, b_mid, mask3);

        let r_shifted = _mm256_slli_epi32(r, 16);
        let g_shifted = _mm256_slli_epi32(g, 8);

        let argb = _mm256_or_si256(
            _mm256_or_si256(c_alpha, r_shifted),
