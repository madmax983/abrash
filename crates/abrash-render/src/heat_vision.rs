...
unsafe fn apply_heat_vision_simd(pixels: &mut [u32], depths: &[f32], min_z: f32, scale: f32) {
    ...
    while i + 8 <= len {
        ...
        // r = min(max(511 - t, 0), 255)
        let r1 = _mm256_max_epi32(_mm256_sub_epi32(c511, t_clamped), zero_vec);
        let r = _mm256_min_epi32(r1, c255);

        // g = min(min(t, 767 - t), 255) (valid because t is positive and clamped)
        let g1 = _mm256_min_epi32(
            t_clamped,
            _mm256_max_epi32(_mm256_sub_epi32(c1023, t_clamped), zero_vec),
        );
        let g = _mm256_min_epi32(g1, c255);

        // b = min(max(t - 512, 0), 255)
        let b1 = _mm256_max_epi32(_mm256_sub_epi32(t_clamped, c512), zero_vec);
        let b = _mm256_min_epi32(b1, c255);
        ...
        let gathered =
            _mm256_or_si256(alpha, _mm256_or_si256(r_shift, _mm256_or_si256(g_shift, b)));
