...
unsafe fn apply_heat_vision_simd(
    ...
    use std::arch::x86_64::{
        ...
        _mm256_storeu_si256, _mm256_sub_ps, _mm256_sub_epi32, _mm256_slli_epi32,
        _mm256_or_si256,
    };
    ...
    let val_511 = _mm256_set1_epi32(511);
    ...
    while i + 8 <= len {
        ...
        // SIMD Arithmetic calculation to replace gather instruction
        // r2 = (511_i32 - t).clamp(0, 255)
        let r_sub = _mm256_sub_epi32(val_511, t_clamped);
        let r_max = _mm256_max_epi32(r_sub, zero_vec);
        let r_clamped = _mm256_min_epi32(r_max, val_255);

        // g2 = t.min(1023 - t).clamp(0, 255)
        let g_sub = _mm256_sub_epi32(max_t_vec, t_clamped);
        let g_min = _mm256_min_epi32(t_clamped, g_sub);
        let g_max = _mm256_max_epi32(g_min, zero_vec);
        let g_clamped = _mm256_min_epi32(g_max, val_255);

        // b2 = (t - 512).clamp(0, 255)
        let b_sub = _mm256_sub_epi32(t_clamped, val_512);
        let b_max = _mm256_max_epi32(b_sub, zero_vec);
        let b_clamped = _mm256_min_epi32(b_max, val_255);

        // Combine into ARGB: (0xFF00_0000 | (r << 16) | (g << 8) | b)
        let r_shifted = _mm256_slli_epi32::<16>(r_clamped);
        let g_shifted = _mm256_slli_epi32::<8>(g_clamped);
        let rg_combined = _mm256_or_si256(r_shifted, g_shifted);
        let rgb_combined = _mm256_or_si256(rg_combined, b_clamped);
        let final_color_arithmetic = _mm256_or_si256(rgb_combined, alpha_mask);

        // Blend: if is_inf, use bg_color, else use final_color_arithmetic
        let final_color = _mm256_blendv_epi8(final_color_arithmetic, bg_color, is_inf_int);
