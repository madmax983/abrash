...
pub fn apply_heat_vision(fb: &mut Framebuffer, zb: &ZBuffer) {
    ...
    for (pixel, &depth) in pixels.iter_mut().zip(depths.iter()) {
        ...
        // Inline math, completely matching SIMD version
        let mut t = (depth - min_z) * scale;
        if t < 0.0 { t = 0.0; }
        let mut t_u32 = t as u32;
        if t_u32 > 1023 { t_u32 = 1023; }

        // SAFETY: t is strictly clamped to 1023 above, which is within the bounds of the 1024-element LUT.
        *pixel = unsafe { *LUT.get_unchecked(t_u32 as usize) };
    }
}
...
unsafe fn apply_heat_vision_simd(
    ...
    use std::arch::x86_64::{
        ...
        _mm256_cvttps_epi32, _mm256_loadu_ps, _mm256_max_epi32,
        _mm256_min_epi32, _mm256_mul_ps, _mm256_set1_epi32, _mm256_set1_ps, _mm256_setzero_si256,
        _mm256_storeu_si256, _mm256_sub_ps, _mm256_cmpgt_epi32, _mm256_andnot_si256,
        _mm256_or_si256, _mm256_slli_epi32, _mm256_sub_epi32,
    };
    ...
    while i + 8 <= len {
        ...
        // --- Fixed Point SIMD Logic ---
        // Instead of gathering from a LUT, calculate the segments inline:
        // Segment 0 (0..255): R=255, G=t, B=0
        // Segment 1 (256..511): R=255-(t-256), G=255, B=0
        // Segment 2 (512..767): R=0, G=255, B=t-512
        // Segment 3 (768..1023): R=0, G=255-(t-768), B=255
        ...
        // Segment masks
        let mask0 = _mm256_cmpgt_epi32(_mm256_set1_epi32(256), t_clamped);
        let mask1_half = _mm256_cmpgt_epi32(_mm256_set1_epi32(512), t_clamped);
        let mask1 = _mm256_andnot_si256(mask0, mask1_half);
        let mask2_half = _mm256_cmpgt_epi32(_mm256_set1_epi32(768), t_clamped);
        let mask2 = _mm256_andnot_si256(mask1_half, mask2_half);
        let mask3 = _mm256_andnot_si256(mask2_half, _mm256_set1_epi32(-1)); // Everything else

        // Seg 0: R=255, G=t, B=0
        let r0 = c255;
        let g0 = t_clamped;
        let b0 = c0;
        let color0 = _mm256_or_si256(
            alpha,
            _mm256_or_si256(
                _mm256_slli_epi32(r0, 16),
                _mm256_slli_epi32(g0, 8),
            )
        );

        // Seg 1: R=255-(t-256), G=255, B=0
        let local_t1 = _mm256_sub_epi32(t_clamped, _mm256_set1_epi32(256));
        let r1 = _mm256_sub_epi32(c255, local_t1);
        let g1 = c255;
        let b1 = c0;
        let color1 = _mm256_or_si256(
            alpha,
            _mm256_or_si256(
                _mm256_slli_epi32(r1, 16),
                _mm256_slli_epi32(g1, 8),
            )
        );

        // Seg 2: R=0, G=255, B=t-512
        let local_t2 = _mm256_sub_epi32(t_clamped, _mm256_set1_epi32(512));
        let r2 = c0;
        let g2 = c255;
        let b2 = local_t2;
        let color2 = _mm256_or_si256(
            alpha,
            _mm256_or_si256(
                _mm256_slli_epi32(g2, 8),
                b2,
            )
        );

        // Seg 3: R=0, G=255-(t-768), B=255
        let local_t3 = _mm256_sub_epi32(t_clamped, _mm256_set1_epi32(768));
        let r3 = c0;
        let g3 = _mm256_sub_epi32(c255, local_t3);
        let b3 = c255;
        let color3 = _mm256_or_si256(
            alpha,
            _mm256_or_si256(
                _mm256_slli_epi32(g3, 8),
                b3,
            )
        );

        // Blend segments together using masks
        let gathered = _mm256_blendv_epi8(
            _mm256_blendv_epi8(
                _mm256_blendv_epi8(color3, color2, mask2),
                color1,
                mask1,
            ),
            color0,
