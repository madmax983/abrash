...
unsafe fn apply_heat_vision_simd(
    ...
    let inf_bits_vec = _mm256_set1_epi32(0x7F80_0000);
    ...
    while i + 8 <= len {
        ...
        let is_inf_int = _mm256_cmpeq_epi32(depth_bits, inf_bits_vec);
...
mod tests {
    ...
    #[test]
    fn test_heat_vision_infinity_comparison_accuracy() {
        // Test specifically that the infinity background comparison works
        // both with float == f32::INFINITY and bitwise depth.to_bits() == 0x7F80_0000
        let mut fb = Framebuffer::new(3, 1).unwrap();
        let mut zb = ZBuffer::new(3, 1).unwrap();

        zb.test_and_set(0, 0, 10.0);
        zb.test_and_set(1, 0, f32::INFINITY);
        zb.test_and_set(2, 0, 20.0);
        ...
        assert_eq!(
            p1, 0xFF00_0010,
            "Infinity pixel should be very dark blue background"
        );
    }
}
