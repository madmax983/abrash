...
pub fn apply_heat_vision(fb: &mut Framebuffer, zb: &ZBuffer) {
    ...
    for (pixel, &depth) in pixels.iter_mut().zip(depths.iter()) {
        ...
        let r = 511u32.saturating_sub(t).min(255);
        let g = t.min(1023 - t).min(255);
        let b = t.saturating_sub(512).min(255);
...
unsafe fn apply_heat_vision_simd(pixels: &mut [u32], depths: &[f32], min_z: f32, scale: f32) {
    ...
    use std::arch::x86_64::{
        ...
        _mm256_cvttps_epi32, _mm256_loadu_ps, _mm256_max_epi32, _mm256_min_epi32, _mm256_mul_ps,
    ...
    let max_color_vec = _mm256_set1_epi32(255);
    ...
    while i + 8 <= len {
        ...
        // r = (511 - t).max(0).min(255)
        let r_sub = _mm256_sub_epi32(c511, t);
        let r_clamped = _mm256_min_epi32(_mm256_max_epi32(r_sub, zero_vec), max_color_vec);

        // g = min(t, 1023 - t).max(0).min(255)
        let g_sub = _mm256_sub_epi32(c1023, t);
        let g_min_t = _mm256_min_epi32(t, g_sub);
        let g_clamped = _mm256_min_epi32(_mm256_max_epi32(g_min_t, zero_vec), max_color_vec);

        // b = (t - 512).max(0).min(255)
        let b_sub = _mm256_sub_epi32(t, c512);
        let b_clamped = _mm256_min_epi32(_mm256_max_epi32(b_sub, zero_vec), max_color_vec);

        // Combine colors: alpha_mask | (r << 16) | (g << 8) | b
        let r_shifted = _mm256_slli_epi32::<16>(r_clamped);
        let g_shifted = _mm256_slli_epi32::<8>(g_clamped);

        let color_rg = _mm256_or_si256(r_shifted, g_shifted);
        let color_rgb = _mm256_or_si256(color_rg, b_clamped);
        let final_rgb = _mm256_or_si256(color_rgb, alpha_mask);

        // Blend: if is_inf, use bg_color, else use final_rgb
        let final_color = _mm256_blendv_epi8(final_rgb, bg_color, is_inf_int);
