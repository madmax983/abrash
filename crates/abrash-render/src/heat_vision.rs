...
unsafe fn apply_heat_vision_simd(
    ...
    use std::arch::x86_64::{
        ...
        _mm256_storeu_si256, _mm256_sub_ps, _mm256_and_si256, _mm256_sub_epi32, _mm256_slli_epi32,
        _mm256_or_si256,
    };
    ...
    while i + 8 <= len {
        ...
        // Instead of gather, calculate r, g, b from t_clamped
        let local_t = _mm256_and_si256(t_clamped, c255);
        let inv_t = _mm256_sub_epi32(c255, local_t);

        // Masks for segments: Note that _mm256_cmpgt_epi32(a, b) means a > b.
        // So c256 > t_clamped is equivalent to t_clamped < 256.
        let is_seg0 = _mm256_cmpgt_epi32(c256, t_clamped);
        let is_seg1_or_less = _mm256_cmpgt_epi32(c512, t_clamped);
        let is_seg2_or_less = _mm256_cmpgt_epi32(c768, t_clamped);

        // r calculation
        // seg0: 255
        // seg1: inv_t
        // seg2,3: 0
        let r_seg01 = _mm256_blendv_epi8(inv_t, c255, is_seg0);
        let r = _mm256_blendv_epi8(zero_vec, r_seg01, is_seg1_or_less);
        ...
        // seg2: 255
        // seg3: inv_t
        let g_seg01 = _mm256_blendv_epi8(c255, local_t, is_seg0);
        let g_seg23 = _mm256_blendv_epi8(inv_t, c255, is_seg2_or_less);
        let g = _mm256_blendv_epi8(g_seg23, g_seg01, is_seg1_or_less);

        // b calculation
        // seg0,1: 0
        // seg2: local_t
        // seg3: 255
        let b_seg23 = _mm256_blendv_epi8(c255, local_t, is_seg2_or_less);
        let b = _mm256_blendv_epi8(b_seg23, zero_vec, is_seg1_or_less);

        // Pack to ARGB (0xFF00_0000 | r << 16 | g << 8 | b)
        ...
        // Blend: if is_inf, use bg_color, else use generated color
