...
unsafe fn apply_heat_vision_simd(
    ...
    use std::arch::x86_64::{
        ...
        _mm256_cmp_ps, _mm256_cmpgt_epi32, _mm256_cvttps_epi32, _mm256_loadu_ps, _mm256_max_epi32,
    ...
    let c767 = _mm256_set1_epi32(767);
    ...
    while i + 8 <= len {
        ...
        // Compute segments
        // seg1: t < 256 -> (t <= 255) -> !(t > 255)
        let is_seg1 = _mm256_cmpgt_epi32(c256, t_clamped);
        let is_seg2 = _mm256_and_si256(
            _mm256_cmpgt_epi32(t_clamped, c255),
            _mm256_cmpgt_epi32(c512, t_clamped),
        );
        let is_seg3 = _mm256_and_si256(
            _mm256_cmpgt_epi32(t_clamped, c511),
            _mm256_cmpgt_epi32(c768, t_clamped),
        );
        let is_seg4 = _mm256_cmpgt_epi32(t_clamped, c767);

        // Compute local_t for each segment
        let local_t_seg2 = _mm256_sub_epi32(t_clamped, c256);
        let local_t_seg3 = _mm256_sub_epi32(t_clamped, c512);
        let local_t_seg4 = _mm256_sub_epi32(t_clamped, c768);

        // Segment 1: r=255, g=t, b=0
        let r1 = c255;
        let g1 = t_clamped;
        let b1 = _mm256_setzero_si256();

        // Segment 2: r=255-local_t, g=255, b=0
        let r2 = _mm256_sub_epi32(c255, local_t_seg2);
        let g2 = c255;
        let b2 = _mm256_setzero_si256();

        // Segment 3: r=0, g=255, b=local_t
        let r3 = _mm256_setzero_si256();
        let g3 = c255;
        let b3 = local_t_seg3;

        // Segment 4: r=0, g=255-local_t, b=255
        let r4 = _mm256_setzero_si256();
        let g4 = _mm256_sub_epi32(c255, local_t_seg4);
        let b4 = c255;

        // Blend R, G, B channels
        let r = _mm256_blendv_epi8(
            _mm256_blendv_epi8(_mm256_blendv_epi8(r4, r3, is_seg3), r2, is_seg2),
            r1,
            is_seg1,
        );

        let g = _mm256_blendv_epi8(
            _mm256_blendv_epi8(_mm256_blendv_epi8(g4, g3, is_seg3), g2, is_seg2),
            g1,
            is_seg1,
        );

        let b = _mm256_blendv_epi8(
            _mm256_blendv_epi8(_mm256_blendv_epi8(b4, b3, is_seg3), b2, is_seg2),
            b1,
            is_seg1,
        );

        // Pack channels into ARGB: (0xFF << 24) | (r << 16) | (g << 8) | b
