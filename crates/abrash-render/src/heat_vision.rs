...
unsafe fn apply_heat_vision_simd(pixels: &mut [u32], depths: &[f32], min_z: f32, scale: f32) {
    ...
    use std::arch::x86_64::{
        __m256i, _CMP_EQ_OQ, _mm256_andnot_si256, _mm256_blendv_epi8, _mm256_castps_si256,
    ...
    while i + 8 <= len {
        ...
        let seg0_mask = _mm256_cmpgt_epi32(_mm256_set1_epi32(256), t_clamped);
        let seg1_mask = _mm256_andnot_si256(
            seg0_mask,
            _mm256_cmpgt_epi32(_mm256_set1_epi32(512), t_clamped),
        );
        let seg2_mask = _mm256_andnot_si256(
            _mm256_or_si256(seg0_mask, seg1_mask),
            _mm256_cmpgt_epi32(_mm256_set1_epi32(768), t_clamped),
        );

        let r1 = _mm256_sub_epi32(_mm256_set1_epi32(511), t_clamped);
        let b2 = _mm256_sub_epi32(t_clamped, _mm256_set1_epi32(512));
        let g3 = _mm256_sub_epi32(_mm256_set1_epi32(1023), t_clamped);

        let r0_r1 = _mm256_blendv_epi8(r1, _mm256_set1_epi32(255), seg0_mask);
        let r = _mm256_blendv_epi8(zero_vec, r0_r1, _mm256_or_si256(seg0_mask, seg1_mask));

        let g0_g12 = _mm256_blendv_epi8(_mm256_set1_epi32(255), t_clamped, seg0_mask);
        let g = _mm256_blendv_epi8(
            g3,
            g0_g12,
            _mm256_or_si256(seg0_mask, _mm256_or_si256(seg1_mask, seg2_mask)),
        );

        let b2_b3 = _mm256_blendv_epi8(_mm256_set1_epi32(255), b2, seg2_mask);
        ...
        let color = _mm256_or_si256(
            _mm256_set1_epi32(0xFF00_0000_u32 as i32),
            _mm256_or_si256(
                _mm256_slli_epi32::<16>(r),
                _mm256_or_si256(_mm256_slli_epi32::<8>(g), b),
            ),
        );

        let final_color = _mm256_blendv_epi8(color, bg_color, is_inf_int);
