...
pub struct TileBins {
    ...
    pub tris: Vec<u32>,
    /// Current generation used to avoid O(N) memset on clear.
    pub current_generation: u32,
    /// Generation array tracking the last time each bin was cleared.
...
impl TileBins {
    ...
    pub fn push(&mut self, tile_idx: usize, tri_idx: usize) {
        ...
        let is_empty = self.generations[tile_idx] != self.current_generation;
        if is_empty {
...
impl TileRenderer {
    ...
    fn sort_bins_gouraud(&mut self) {
        ...
        for (tile_idx, head) in heads.iter_mut().enumerate() {
            if self.tile_bins.generations[tile_idx] != self.tile_bins.current_generation {
                continue;
            }
