...
///
/// # Optimization
/// We use conditional checks before plotting symmetric points (e.g., `if x != 0`)
/// to prevent redundant pixel writes when points fall on axes or diagonals.
/// This prevents overdraw and measurably improves rasterization performance.
...
mod tests {
    ...
    fn test_circle_radius_too_large() {
        ...
        draw_circle(&mut fb, 50, 50, 16385, 0x00FF_FFFFFF);
        fill_circle(&mut fb, 50, 50, 16385, 0x00FF_FFFFFF);
        // Test with massive values
        draw_circle(&mut fb, 50, 50, i32::MAX / 2 + 2, 0x00FF_FFFFFF);
        fill_circle(&mut fb, 50, 50, i32::MAX / 2 + 2, 0x00FF_FFFFFF);
        draw_circle(&mut fb, 50, 50, i32::MAX, 0x00FF_FFFFFF);
        fill_circle(&mut fb, 50, 50, i32::MAX, 0x00FF_FFFFFF);
    }
