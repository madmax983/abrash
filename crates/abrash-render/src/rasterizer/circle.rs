...
pub fn fill_circle(fb: &mut Framebuffer, xc: i32, yc: i32, radius: i32, color: u32) {
    ...
    if min_x >= 0 && max_x < i64::from(fb.width()) && min_y >= 0 && max_y < i64::from(fb.height()) {
        ...
        while y >= x {
            ...
            if y != last_y {
                if last_y > x {
                    draw_horizontal_line(fb, xc.saturating_sub(x), xc.saturating_add(x), yc.saturating_add(last_y), color);
                    if last_y > 0 {
                        draw_horizontal_line(fb, xc.saturating_sub(x), xc.saturating_add(x), yc.saturating_sub(last_y), color);
                    }
                }
                last_y = y;
            }
