...
proptest! {
    ...
    #[test]
    #[ignore = "👹 Havoc: Direct Fuzzing of unsafe SIMD trilinear fast mapping"]
    fn test_draw_span_textured_gouraud_simd_fuzz(
        z in any::<f32>(),
        dz_dx in any::<f32>(),
        u_fix in any::<i32>(),
        v_fix in any::<i32>(),
        ...
        use crate::rasterizer::texture::draw_span_textured_gouraud_simd;
        let mut fb = vec![0u32; fb_len];
        let mut zb = vec![100.0f32; zb_len];
        let mut tex = Texture::new(w, h).unwrap();
        tex.set_pixel(0, 0, 0x00FF_FFFFFF);
        ...
        unsafe {
            draw_span_textured_gouraud_simd(
                &mut fb, &mut zb, &tex, z, dz_dx, u_fix, v_fix, du_fix, dv_fix,
                r_start, g_start, b_start, dr_dx, dg_dx, db_dx
            );
        }
    }
}
