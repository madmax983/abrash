...
impl Scene {
    ...
    pub fn extract_into(&self, draw_list: &mut DrawList) {
        ...
        RENDER_CONTEXT.with(|ctx_cell| {
            ...
            world_aabbs.clear();
            // ⚡ Bolt: Use `reserve` to leverage amortized O(1) growth and prevent
            // frequent O(N) reallocations when capacities fluctuate frame-to-frame.
            ...
            // Use `reserve` to leverage amortized O(1) growth and prevent frequent O(N) reallocations.
            draw_list.batches.reserve(visible_count);
            draw_list.vertices.reserve(total_vertices);
