...
pub struct DrawBatch {
    /// Range of vertices in the parent `DrawList`'s `vertices` array.
    ///
...
pub struct DrawList {
    ...
    /// Single, contiguous flat array of all clip-space vertices `(position, w)` for this frame.
    ///
    /// ⚡ Bolt Performance Optimization:
    /// Replaces the `Vec<(Vec3, f32)>` per `DrawBatch` with a flat array,
    /// allowing a single O(1) allocation per frame instead of N heap allocations per mesh.
