...
impl<T> ResourcePool<T> {
    ...
    pub fn insert(&mut self, value: T) -> Handle<T> {
        if let Some(index) = self.free_list.pop() {
            ...
            assert!(entry.value.is_none(), "free list pointed to occupied slot");
            ...
            let generation = 0;
            self.entries.push(PoolEntry { value: Some(value), generation });
            Handle::new(index, generation)
        }
    }
    ...
    pub fn get(&self, handle: Handle<T>) -> Option<&T> {
        self.entries
            ...
            .and_then(|entry| if entry.generation == handle.generation { entry.value.as_ref() } else { None })
    }
    ...
    pub fn get_mut(&mut self, handle: Handle<T>) -> Option<&mut T> {
        self.entries
            ...
            .and_then(|entry| if entry.generation == handle.generation { entry.value.as_mut() } else { None })
    }
...
mod tests {
    ...
    fn test_pool_free_list_occupied_slot_panic() {
        ...
        pool.entries[h1.index as usize] = PoolEntry {
            value: Some(99),
