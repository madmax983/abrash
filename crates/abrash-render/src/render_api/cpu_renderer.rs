...
impl CpuRenderer {
    ...
    pub fn extract_draw_list_into(
        ...
        // ⚡ Bolt: Use `clone_from` instead of `clear()` followed by `extend_from_slice()` to idiomatically
        // retain the capacity of the lights vector, eliminating heap allocations for identical frames.
...
mod tests {
    ...
    #[test]
    fn test_extract_draw_list_stale_mesh_handle_error() {
        let mut renderer = CpuRenderer::new(100, 100);
        let mesh_h = renderer.create_mesh(&Mesh::cube(1.0)).unwrap();
        let mat_h = renderer
            .create_material(Material::flat(0xFFFF_0000))
