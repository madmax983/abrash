...
impl CpuRenderer {
    ...
    pub fn extract_draw_list(&self, frame: &Frame) -> Result<DrawList, RenderError> {
        ...
        // ⚡ Bolt: Pre-validate all handles sequentially to eliminate intermediate Result<Vec, Error>
        // allocations during the parallel pass.
        ...
        #[cfg(feature = "parallel")]
        {
            ...
            // ⚡ Bolt: Use `par_extend` combined with an infallible map to write directly
            // into `draw_list.batches`, entirely eliminating O(N) heap allocations per frame.
            draw_list
                .batches
                .par_extend(
                    frame
                        ...
                        .zip(&ranges)
                        .map(|(cmd, &(start, end))| {
                            // Safe to unwrap since we pre-validated all handles in the sequential loop above.
                            let cpu_mesh = self.meshes.get(from_mesh_handle(cmd.mesh)).unwrap();
                            let material = self
                                .materials
                                .get(from_material_handle(cmd.material))
