...
impl Renderer for CpuRenderer {
    ...
    fn update_mesh(&mut self, handle: MeshHandle, mesh: &Mesh) -> Result<(), RenderError> {
        ...
        // By using `clone_from` instead of `= mesh.clone()`, we reuse the existing `Vec` capacities
        // inside the CPU mesh (vertices, indices, normals, etc.), eliminating multiple O(N)
        // heap allocations per frame when updating dynamic geometry like skinned characters.
