...
fn intersect_scene_triangle(
    ...
    let point = ray.at(t);
    // Approximate UVs if needed, or re-calculate using MT
...
impl RayTracer {
    ...
    fn trace_ray(&self, ray: &Ray, objects: &[SceneObject], aabbs: &[AABB], depth: u32) -> u32 {
        ...
        for (obj, world_aabb) in objects.iter().zip(aabbs.iter()) {
            if !ray.intersect_aabb(world_aabb).map_or(false, |(enter, e)| e >= enter && e > 0.001 && enter < closest_t) {
                continue;
            }
            ...
            for indices in &mesh.indices {
                ...
                if let Some(hit) = intersect_scene_triangle(&ray, v0, v1, v2, 0.001, closest_t) {
                    closest_t = hit.t;
    ...
    fn check_shadow(ray: &Ray, objects: &[SceneObject], aabbs: &[AABB]) -> bool {
        for (obj, world_aabb) in objects.iter().zip(aabbs.iter()) {
            if !ray.intersect_aabb(world_aabb).map_or(false, |(enter, e)| e >= enter && e > 0.001 && enter < 1000.0) {
                continue;
            }
            let mesh = &obj.mesh;
            for indices in &mesh.indices {
                ...
                if intersect_scene_triangle(ray, v0, v1, v2, 0.001, 1000.0).is_some() {
                    return true;
                }
            }
        }
