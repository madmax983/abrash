            ...
            TRAIL_MAP_NEXT.with(|trail_map_next| {
                ...
                for agent in agents_borrow.iter_mut() {
                    let sense = |angle_offset: f32, trail_slice: &[f32]| -> f32 {
                        ...
                        let sensor_dir = Vec2::new(sensor_angle.cos(), sensor_angle.sin());
                        let sensor_pos = Vec2::new(
                            agent.position.x + sensor_dir.x * config.sensor_offset_dist,
                            agent.position.y + sensor_dir.y * config.sensor_offset_dist,
                        );

                        let sx = sensor_pos.x as i32;
                        let sy = sensor_pos.y as i32;
                    ...
                    agent.position.x = agent.position.x.rem_euclid(width as f32);
                    agent.position.y = agent.position.y.rem_euclid(height as f32);
                    ...
                    for x in 0..width {
                        ...
                        for dy in -1..=1 {
                            for dx in -1..=1 {
                                ...
                                sum += trail_borrow[ny * width + nx] * weight;
                            }
                        }
