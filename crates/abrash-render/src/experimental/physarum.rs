...
fn update_agents(
    agents_borrow: &mut [Agent],
    trail_borrow: &mut [f32],
    width: usize,
    height: usize,
...
fn diffuse_and_decay(
    trail_borrow: &mut std::vec::Vec<f32>,
    trail_next_borrow: &mut std::vec::Vec<f32>,
    width: usize,
    height: usize,
    config: &PhysarumConfig,
) {
    ...
    // Swap buffers
    std::mem::swap(trail_borrow, trail_next_borrow);
}

fn initialize_simulation(
    agents_borrow: &mut Vec<Agent>,
    trail_borrow: &mut Vec<f32>,
    trail_next_borrow: &mut Vec<f32>,
    grid_size: usize,
    width: usize,
    height: usize,
    config: &PhysarumConfig,
    ...
    AGENTS.with(|agents| {
        TRAIL_MAP.with(|trail_map| {
            TRAIL_MAP_NEXT.with(|trail_map_next| {
                ...
                update_agents(&mut agents_borrow, &mut trail_borrow, width, height, config);
                ...
                render_trail_map(fb.as_mut_slice(), &trail_borrow, grid_size, config);
            });
        });
    });
}
