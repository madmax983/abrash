    ...
    AGENTS.with(|agents| {
        TRAIL_MAP.with(|trail_map| {
            TRAIL_MAP_NEXT.with(|trail_map_next| {
                ...
                update_agents(&mut agents_borrow, &mut trail_borrow, config, width, height);

                diffuse_and_decay(&trail_borrow, &mut trail_next_borrow, config, width, height);

                // Swap buffers
                std::mem::swap(&mut *trail_borrow, &mut *trail_next_borrow);

                render_physarum(fb, &trail_borrow, config, grid_size);
            });
        });
    });
}
