...
pub fn apply_paper_cutout(fb: &mut Framebuffer, zb: &ZBuffer, config: &PaperCutoutConfig) {
    ...
    // Clone framebuffer so we can read un-darkened pixels while writing shadows
    let src_fb = match Framebuffer::new(width as u32, height as u32) {
        Ok(mut f) => {
            f.as_mut_slice().copy_from_slice(pixels);
            f
        }
        Err(_) => return, // OOM handling
    };

    let src_pixels = src_fb.as_slice();

    // Quantize layers
    let layers = config.layers.max(1) as f32;
    ...
    iter.for_each(|(y, row)| {
        ...
        for (x, pixel) in row.iter_mut().enumerate() {
            ...
            // Background remains background, but can receive shadows
            let current_layer = if current_depth == f32::INFINITY {
                layers as i32 // Background is the deepest layer
            } else {
                let normalized = (current_depth - min_z) * inv_range;
                (normalized * layers) as i32
            };

            // Read the source color for this pixel
            let mut color = src_pixels[idx];
            ...
            if shadow_x >= 0 && shadow_x < width as i32 && shadow_y >= 0 && shadow_y < height as i32
            {
                ...
                let caster_depth = depths[caster_idx];

                let caster_layer = if caster_depth == f32::INFINITY {
                    layers as i32
                } else {
                    let normalized = (caster_depth - min_z) * inv_range;
                    (normalized * layers) as i32
                };
