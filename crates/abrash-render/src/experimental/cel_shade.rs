...
fn quantize_color(color: u32, levels: u32) -> u32 {
    ...
    let levels_minus_1 = u64::from(levels - 1);
    let safe_divisor = levels_minus_1.max(1);

    let a = color & 0xFF00_0000;
    let r = u64::from((color >> 16) & 0xFF);
    let g = u64::from((color >> 8) & 0xFF);
    let b = u64::from(color & 0xFF);

    // ⚡ Bolt: Replace floating-point arithmetic with scaled integer math
    let qr = (((r * levels_minus_1 + 127) / 255 * 255) / safe_divisor) as u32;
    let qg = (((g * levels_minus_1 + 127) / 255 * 255) / safe_divisor) as u32;
    let qb = (((b * levels_minus_1 + 127) / 255 * 255) / safe_divisor) as u32;
