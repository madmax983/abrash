...
const fn quantize_color(color: u32, levels: u32) -> u32 {
    if levels <= 1 {
    ...
    let levels_minus_1 = (levels - 1) as u64;

    // (r * levels_minus_1 + 127) / 255 * 255 / levels_minus_1
    // To avoid overflow, we cast to u64 and do math
    // ⚡ Bolt: Replace floating-point arithmetic and rounding with pure scaled integer math
    let qr = ((((r * levels_minus_1 + 127) / 255) * 255) / levels_minus_1) as u32;
    let qg = ((((g * levels_minus_1 + 127) / 255) * 255) / levels_minus_1) as u32;
    let qb = ((((b * levels_minus_1 + 127) / 255) * 255) / levels_minus_1) as u32;
