...
pub fn encode_message(fb: &mut Framebuffer, message: &str) -> Result<(), Error> {
    ...
    let total_bytes_needed = 4_usize
        ...
        .ok_or_else(|| Error::CapacityExceeded("Message too large".to_string()))?;

    // Each pixel can store 3 bits (one in R, one in G, one in B).
    let bits_needed = total_bytes_needed
        .checked_mul(8)
        .ok_or_else(|| Error::CapacityExceeded("Message too large".to_string()))?;
    let pixels_needed = bits_needed
        .checked_add(2)
        .ok_or_else(|| Error::CapacityExceeded("Message too large".to_string()))?
        / 3;
    ...
    if pixels_needed > (fb.width() * fb.height()) as usize {
        return Err(Error::CapacityExceeded(
            "Framebuffer too small to hold the message".to_string(),
        ));
    }
