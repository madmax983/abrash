...
pub fn apply_hex_mosaic(fb: &mut Framebuffer, cell_size: f32, border_size: f32, border_color: u32) {
    ...
    let inv_grid_w = 1.0 / grid_w;
    let inv_grid_h = 1.0 / grid_h;
        ...
        pixels
            ...
            .for_each(|(y, row)| {
                ...
                for x in 0..width {
                    ...
                    // Grid 1
                    let grid_x1 = fast_round(px * inv_grid_w);
                    let grid_y1 = fast_round(py * inv_grid_h);
                    let cx1 = grid_x1 * grid_w;
                    let cy1 = grid_y1 * grid_h;

                    // Grid 2
                    let grid_x2 = fast_round((px - 1.5 * r) * inv_grid_w);
                    let grid_y2 = fast_round((py - 0.5 * grid_h) * inv_grid_h);
                    ...
                    let center_x = fast_round(cx) as isize;
                    let center_x = center_x.clamp(0, (width - 1) as isize) as usize;
                    let center_y = fast_round(cy) as isize;
                    let center_y = center_y.clamp(0, (height - 1) as isize) as usize;
                    ...
                    if hex_dist > inradius - border_size {
                        unsafe {
                            *row.get_unchecked_mut(x) = border_color;
                        }
                    } else {
                        unsafe {
                            *row.get_unchecked_mut(x) =
                                *src_pixels.get_unchecked(center_y * width + center_x);
                        }
                    }
                }
            });
    ...
    #[cfg(not(feature = "parallel"))]
    {
        for y in 0..height {
            ...
            for x in 0..width {
                ...
                if hex_dist > inradius - border_size {
                    unsafe {
                        *pixels.get_unchecked_mut(y * width + x) = border_color;
                    }
                } else {
                    unsafe {
                        *pixels.get_unchecked_mut(y * width + x) =
                            *src_pixels.get_unchecked(center_y * width + center_x);
                    }
                }
            }
