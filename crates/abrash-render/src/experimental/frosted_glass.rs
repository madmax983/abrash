...
pub fn apply_frosted_glass(fb: &mut Framebuffer, intensity: f32, seed: u64) {
    ...
    // ⚡ Bolt: Use a thread-local static `RefCell` to eliminate a dynamic heap allocation
            ...
            dest_pixels
                ...
                .for_each(|(y, row)| {
                    ...
                    for x in 0..width {
                        ...
                        row[x as usize] = src_pixels_slice[src_idx];
                    }
                });
        }
        ...
        #[cfg(not(feature = "parallel"))]
        {
            ...
            for y in 0..height {
                ...
                for x in 0..width {
                    ...
                    let src_idx = (sy * width + sx) as usize;
                    dest_pixels[row_start + x as usize] = src_pixels_slice[src_idx];
                }
            }
        }
    });
