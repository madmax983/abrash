...
impl SoftBody {
    ...
    ///
    /// # Errors
    /// Returns an error if spring indices are out of bounds.
    pub fn add_spring(
        &mut self,
        index_a: usize,
        index_b: usize,
        ...
        if index_a >= max_idx || index_b >= max_idx {
            return Err(crate::experimental::error::Error::OutOfBounds(format!(
    ...
    ///
    /// # Errors
    /// Returns an error if spring indices or mesh indices are out of bounds.
    pub fn new(
        mesh: Mesh,
        mass: f32,
        stiffness: f32,
        damping: f32,
    ) -> Result<Self, crate::experimental::error::Error> {
