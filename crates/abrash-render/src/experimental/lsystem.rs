...
impl LSystem {
    ...
    pub fn expand(&self, iterations: usize) -> Result<String, crate::experimental::error::Error> {
        if iterations == 0 {
        ...
        if is_pure_ascii {
            ...
            return LSYSTEM_BUFFERS.with(|bufs| {
                ...
                for _ in 0..iterations {
                    ...
                    for &b in &*current_bytes {
                        ...
                        if next_bytes.len() > self.max_capacity {
                            return Err(crate::experimental::error::Error::CapacityExceeded("L-System expansion exceeded maximum capacity limit"));
                        }
                    }
                ...
                std::str::from_utf8(current_bytes)
                    ...
                    .map_err(|_| crate::experimental::error::Error::InvalidInput("L-System utf8 decoding error"))
            });
        }
...
impl Turtle {
    ...
    pub fn generate_mesh(&mut self, commands: &str) -> Result<Mesh, crate::experimental::error::Error> {
            ...
            match c {
                ...
                b'[' => {
                    if self.stack.len() > 1000 {
                        return Err(crate::experimental::error::Error::CapacityExceeded("L-System stack overflow"));
                    }
                ...
                b']' => {
                    if let Some(state) = self.stack.pop() {
                        ...
                        return Err(crate::experimental::error::Error::CapacityExceeded("L-System stack underflow"));
                    }
                }
