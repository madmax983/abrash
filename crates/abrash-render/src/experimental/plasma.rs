...
mod tests {
    ...
    #[test]
    fn test_no_modulo_1_in_plasma() {
        let path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/experimental/plasma.rs");
        let content = std::fs::read_to_string(path).unwrap();
        let pattern = format!("{} {}", "%", "1.0");
        assert!(
            !content.contains(&pattern),
            "Floating-point modulo is a performance bottleneck."
        );
    }
}
