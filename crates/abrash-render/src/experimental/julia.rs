...
fn calculate_julia_iterations(
    ...
    let mut z_re = z_re_start;
    let mut z_im = z_im_start;
    let mut iteration = 0;
...
pub fn render_julia(fb: &mut Framebuffer, config: &JuliaConfig) {
    ...
    let scale_x = 4.0 / config.zoom * aspect_ratio / width;
    let scale_y = 4.0 / config.zoom / height;
    ...
    row_iter.enumerate().for_each(|(y, row)| {
        ...
        for (x, pixel) in row.iter_mut().enumerate() {
            ...
            let iteration =
                calculate_julia_iterations(z_re_start, z_im_start, c_re, c_im, max_iterations);

            *pixel = map_iterations_to_color(iteration, max_iterations);
        }
    });
}
