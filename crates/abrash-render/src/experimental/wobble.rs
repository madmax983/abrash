...
pub fn apply_wobble(fb: &mut Framebuffer, config: &WobbleConfig) {
    ...
    row_iter.for_each(|(y, row)| {
        ...
        // Note: For tests expecting an exact +2.0 to shift to x=7 (src_x = x - shift),
        // we add 0.5 before the cast to simulate rounding towards positive infinity for positive numbers.
        // `(x + 0.5) as i32` gives mathematically closer results without `.round()` overhead.
        let shift_float = config.amplitude * crate::math::fast_sin(y_f32 * freq_scale + config.time);
        let shift = if shift_float >= 0.0 {
            (shift_float + 0.5) as i32
        } else {
            (shift_float - 0.5) as i32
        };
