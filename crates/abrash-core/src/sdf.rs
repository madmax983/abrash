...
pub fn rounded_cylinder_3d(p: Vec3, centre: Vec3, ra: f32, rb: f32, h: f32) -> f32 {
    ...
    let d = max_xz.mul_add(max_xz, max_y * max_y).sqrt() + d_xz.min(0.0).min(d_y.min(0.0)) - rb;
    d
}
...
pub fn link_3d(p: Vec3, centre: Vec3, r1: f32, r2: f32, le: f32) -> f32 {
    ...
    let qy = p.y.abs() - le;
    qx.mul_add(qx, qy.max(0.0) * qy.max(0.0)).sqrt() - r2
}
...
pub fn hexagonal_prism_3d(p: Vec3, centre: Vec3, r: f32, h: f32) -> f32 {
    ...
    let py_r = py - r;
    let cx = px_clamped.mul_add(px_clamped, py_r * py_r).sqrt();
...
pub fn rhombus_3d(p: Vec3, la: f32, lb: f32, h: f32, ra: f32) -> f32 {
    ...
    let side_pt = Vec2::new(sx.mul_add(sx, sz * sz).sqrt(), p.y - h);
