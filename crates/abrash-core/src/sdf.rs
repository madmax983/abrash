...
/// ```
#[must_use]
/// ⚡ Bolt: Using `.cbrt().powi(2)` instead of `.powf(2.0/3.0)` and `.cbrt()` instead
/// of `.powf(1.0/3.0)` avoids expensive C-math library invocations and improves robustness
/// (as `f32::powf` produces `NaN` for negative bases), yielding measurable performance gains.
