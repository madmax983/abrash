...
#[cfg(feature = "nova")]
impl Mesh {
    /// Exports the mesh to a Wavefront OBJ string format.
    ///
    /// This implementation includes vertices, texture coordinates (if available),
    /// normals (if available), and triangular faces.
    #[must_use]
    pub fn to_obj(&self) -> String {
        use std::fmt::Write;

        let mut obj = String::new();
        // Reserve an estimated capacity (e.g., ~50 chars per vertex, ~30 per face)
        let est_capacity = self.vertices.len() * 50 + self.indices.len() * 30;
        obj.reserve(est_capacity);

        writeln!(&mut obj, "# Generated by Abrash (Nova)").unwrap();

        for v in &self.vertices {
            writeln!(&mut obj, "v {x} {y} {z}", x = v.x, y = v.y, z = v.z).unwrap();
        }

        for uv in &self.uvs {
            writeln!(&mut obj, "vt {x} {y}", x = uv.x, y = uv.y).unwrap();
        }

        for n in &self.normals {
            writeln!(&mut obj, "vn {x} {y} {z}", x = n.x, y = n.y, z = n.z).unwrap();
        }
        ...
        for face in &self.indices {
            ...
            if has_uvs && has_normals {
                writeln!(&mut obj, "f {v1}/{v1}/{v1} {v2}/{v2}/{v2} {v3}/{v3}/{v3}").unwrap();
            } else if has_uvs {
                writeln!(&mut obj, "f {v1}/{v1} {v2}/{v2} {v3}/{v3}").unwrap();
            } else if has_normals {
                writeln!(&mut obj, "f {v1}//{v1} {v2}//{v2} {v3}//{v3}").unwrap();
            } else {
                writeln!(&mut obj, "f {v1} {v2} {v3}").unwrap();
            }
        }
...
mod tests_obj_exporter {
    use super::*;

    #[test]
    fn to_obj_simple_triangle() {
        let mut mesh = Mesh::new();
        mesh.vertices.push(Vec3::new(0.0, 0.0, 0.0));
        mesh.vertices.push(Vec3::new(1.0, 0.0, 0.0));
        mesh.vertices.push(Vec3::new(0.0, 1.0, 0.0));
        ...
        assert!(obj.contains("f 1 2 3")); // 1-based indexing
    }

    #[test]
    fn to_obj_with_uvs_and_normals() {
        let mut mesh = Mesh::new();
        mesh.vertices.push(Vec3::new(0.0, 0.0, 0.0));
        mesh.vertices.push(Vec3::new(1.0, 0.0, 0.0));
        mesh.vertices.push(Vec3::new(0.0, 1.0, 0.0));
