...
impl<V> ClippedTriangles<V> {
    ...
    const fn new_uninit() -> Self {
        Self {
            ...
            tris: [const { std::mem::MaybeUninit::uninit() }; 24],
            count: 0,
        }
    }
}
...
pub fn clip_triangle_to_frustum<V: Copy>(
    ...
    let mut buf1: [MaybeUninit<V>; 12] = [const { std::mem::MaybeUninit::uninit() }; 12];
    let mut buf2: [MaybeUninit<V>; 12] = [const { std::mem::MaybeUninit::uninit() }; 12];
...
pub fn clip_triangle_against_near_plane<V: Copy>(
    ...
    let mut out_verts: [MaybeUninit<V>; 4] = [const { std::mem::MaybeUninit::uninit() }; 4];
    let mut out_count = 0;
