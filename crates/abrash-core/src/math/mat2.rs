...
impl Mat2 {
    ...
    /// Transform multiple vectors at once, writing the results into `out`.
    ///
    /// ⚡ **Bolt**: By taking an `out: &mut Vec<Vec2>`, this zero-cost abstraction
    /// completely eliminates dynamic heap allocations across frames by allowing the
    /// caller to reuse an existing vector's capacity.
