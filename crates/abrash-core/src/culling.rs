...
use crate::geometry::{AABB, BoundingSphere, Plane, Frustum};
...
impl Frustum {
    ...
    pub fn cull_spheres_prealloc(&self, spheres: &[BoundingSphere], results: &mut [bool]) {
        ...
        for (i, sphere) in spheres.iter().enumerate() {
            results[i] = self.intersects_sphere(sphere.center, sphere.radius);
        }
    }
    ...
    unsafe fn cull_spheres_avx2(&self, spheres: &[BoundingSphere], results: &mut [bool]) {
        ...
        unsafe {
            ...
            for (j, plane) in self.planes.iter().enumerate() {
                ...
                plane_d[j] = _mm256_set1_ps(plane.d);
            }
        ...
        // Tail
        while i < len {
            results[i] = self.intersects_sphere(spheres[i].center, spheres[i].radius);
            i += 1;
        }
    }
...
mod tests {
    ...
    fn test_cull_spheres_matches_scalar() {
        ...
        for sphere in &spheres {
            scalar_results.push(frustum.intersects_sphere(sphere.center, sphere.radius));
        }
    ...
    fn test_plane_distance() {
        ...
        assert!((plane.signed_distance(p_inside) - 1.0).abs() < 1e-5);

        // Point at (0, 1, 0): d = 1 - 2 = -1 (Outside/Negative)
        let p_outside = Vec3::new(0.0, 1.0, 0.0);
        assert!((plane.signed_distance(p_outside) - (-1.0)).abs() < 1e-5);

        // Point at (0, 2, 0): d = 2 - 2 = 0 (On plane)
        let p_on = Vec3::new(0.0, 2.0, 0.0);
        assert!(plane.signed_distance(p_on).abs() < 1e-5);
    }
    ...
    fn test_frustum_intersection() {
        ...
        assert!(
            frustum.intersects_sphere(s_inside.center, s_inside.radius),
        ...
        assert!(
            frustum.intersects_sphere(s_intersect.center, s_intersect.radius),
        ...
        assert!(
            !frustum.intersects_sphere(s_outside.center, s_outside.radius),
