...
impl GpuBlitter {
    ...
    pub fn upload_atlas(&mut self, texture: &abrash_core::texture::Texture) -> AtlasHandle {
        ...
        thread_local! {
            static UPLOAD_TEXTURE_BUFFER: std::cell::RefCell<Vec<u8>> = const { std::cell::RefCell::new(Vec::new()) };
        }

        UPLOAD_TEXTURE_BUFFER.with(|buf| {
            let mut rgba = buf.borrow_mut();
            let required_len = pixels.len() * 4;
    ...
    fn upload_framebuffer(&self, fb: &abrash_core::framebuffer::Framebuffer) {
        ...
        // ⚡ Bolt: Eliminates O(N) dynamic heap allocation per frame by reusing a thread-local
