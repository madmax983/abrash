...
pub struct GpuRenderer {
    ...
    // Pre-allocated buffers to prevent per-frame heap allocations
    prepared_draws_buffer: Vec<PreparedDraw>,
    draw_bytes_buffer: Vec<u8>,
}
...
impl GpuRenderer {
    ...
    pub fn from_gpu(gpu: GpuDevice, color_format: wgpu::TextureFormat) -> Self {
        ...
        Self {
            ...
            prepared_draws_buffer: Vec::with_capacity(128),
            draw_bytes_buffer: Vec::with_capacity(128 * 256),
        }
    }
    ...
    pub fn capture(
        ...
        let mut prepared_draws = std::mem::take(&mut self.prepared_draws_buffer);
        let mut draw_uniform_bytes = std::mem::take(&mut self.draw_bytes_buffer);

        let total_triangles =
            match self.prepare_draws(frame, &mut prepared_draws, &mut draw_uniform_bytes) {
                Ok(triangles) => triangles,
                Err(e) => {
                    self.prepared_draws_buffer = prepared_draws;
                    self.draw_bytes_buffer = draw_uniform_bytes;
                    return Err(e);
                }
            };
        ...
        Ok(GpuDebugCapture {
            stats: FrameStats {
                width: w,
                height: h,
                batch_count: self.prepared_draws_buffer.len(),
                total_triangles,
    ...
    pub fn render_to_surface(
        ...
        if let Err(e) = self.prepare_draws(frame, &mut prepared_draws, &mut draw_uniform_bytes) {
            self.prepared_draws_buffer = prepared_draws;
            self.draw_bytes_buffer = draw_uniform_bytes;
            return Err(e);
        }
    ...
    fn prepare_draws(
        &self,
        frame: &Frame,
        prepared_draws: &mut Vec<PreparedDraw>,
        draw_bytes: &mut Vec<u8>,
    ) -> Result<u32, String> {
        let draw_uniform_size = std::mem::size_of::<DrawUniforms>();

        prepared_draws.clear();
        let required_bytes =
            (self.draw_uniform_stride * frame.commands.len().max(1) as u64) as usize;
        draw_bytes.clear();
        draw_bytes.resize(required_bytes, 0u8);
