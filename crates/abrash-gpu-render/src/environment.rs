...
impl GpuCubemap {
    ...
    pub fn from_faces(
        ...
        for (layer, face) in faces.iter().enumerate() {
            // ⚡ Bolt: Uses a pre-allocated zeroed vector and zipped chunks to elide bounds checks
            // and eliminate the dynamic capacity overhead present in `extend_from_slice()`.
            let mut rgba = vec![0u8; (size * size * 4) as usize];
            for (chunk, &argb) in rgba.chunks_exact_mut(4).zip(face.pixels.iter()) {
