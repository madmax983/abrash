...
proptest! {
    ...
    fn fuzz_clipping(
        ...
        let lerp = |a: (Vec3, f32), b: (Vec3, f32), t: f32| (a.0.lerp(b.0, t), a.1 + (b.1 - a.1) * t);

        // Run Optimized (SIMD)
        let result_simd = clip_triangle_to_frustum(v0, v1, v2, get_pos, lerp);
        let vec_simd = clipped_to_vec(&result_simd);

        // Run Scalar Oracle
        let vec_scalar = clip_triangle_scalar(v0, v1, v2, get_pos, lerp);
