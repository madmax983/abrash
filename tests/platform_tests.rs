...
fn window_type_alias_resolves() {
    ...
    // We can't construct it easily in a headless test environment, but we can
    // confirm the type is defined and accessible by extracting its size at compile time.
    let _size = std::mem::size_of::<abrash::platform::Window>();
}
