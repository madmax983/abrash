...
fn window_type_alias_resolves() {
    ...
    // but we can confirm the type exists and has the expected methods
    // by checking it at compile time.
    let _window_type_exists: Option<abrash::platform::Window> = None;
}
