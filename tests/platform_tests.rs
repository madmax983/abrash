...
//!
//! These tests verify that the platform abstractions (type alias,
//! enums) are correctly wired up. We cannot create actual Win32 or TUI
//! windows in test mode, so we test types and enum surface only.
...
fn window_type_alias_resolves() {
    ...
    // We check that the type alias exists.
    let _ = std::mem::size_of::<abrash::platform::Window>();
}
