...
mod tests {
    ...
    fn test_framebuffer_rejects_huge_dimensions() {
        ...
        assert!(fb_result.is_err(), "Framebuffer should reject width > i32::MAX");
        assert_eq!(fb_result.err(), Some("Buffer dimensions too large (max i32::MAX)"));

        let height_huge = u32::MAX;
        let width_small = 1;
        let fb_result_h = Framebuffer::new(width_small, height_huge);
        assert!(fb_result_h.is_err(), "Framebuffer should reject height > i32::MAX");
    }
    ...
    fn test_zbuffer_rejects_huge_dimensions() {
        ...
        assert_eq!(zb_result.err(), Some("Buffer dimensions too large (max i32::MAX)"));
    }
}
