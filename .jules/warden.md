...
**2024-05-18 - [Prevent Arboretum OOM DoS]**
**Threat:** L-System strings expanded with millions of `F` segments bypass pre-allocation caps but push dynamically to internal `Vec`s without bounds checking, causing 9.6GB+ OOM panics.
**Defense:** Added explicit `num_segments` counting limit (`max_segments: usize = 1_000_000`) within `generate_mesh` pre-flight loop to reject excessive geometries safely.
