...
## 2026-07-07 - [Vulnerable Dependencies: anyhow, memmap2, crossbeam-epoch]
**Threat:** Found multiple active vulnerabilities in dependencies during `cargo audit` (RUSTSEC-2026-0190 for anyhow, RUSTSEC-2026-0186 for memmap2, and RUSTSEC-2026-0204 for crossbeam-epoch). The anyhow vulnerability involved unsoundness in `Error::downcast_mut()`. The memmap2 vulnerability involved unchecked pointer offset. The crossbeam-epoch vulnerability involved invalid pointer dereferences.
**Defense:** Fixed the vulnerabilities immediately by running `cargo update -p crossbeam-epoch`, `cargo update -p anyhow`, and `cargo update -p memmap2` to bump to the patched versions in Cargo.lock.
