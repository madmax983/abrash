...
**2024-05-18 - Integer Overflow & CPU DoS in Bresenham Circle Rasterization**
**Threat:** Supplying an arbitrarily large radius (e.g., 600,000,000) to `draw_circle` or `fill_circle` bypassed initial bounding box clipping checks (since the bounding box covered the entire screen). Bresenham's inner loop algorithm (`O(R)` complexity) iterated millions of times causing severe CPU Denial of Service. Furthermore, the `d = d + 4 * (x - y) + 10` inner math calculation panicked due to integer overflow of `i32` before the loop could finish, causing an unhandled crash in the rendering process.
**Defense:** Explicit input bounds validation applied at the boundary of `draw_circle` and `fill_circle`. Since viewport dimensions are bounded to 16,384, an early return `if radius > 16384` was added. This safely defends against both the CPU DoS and integer overflows while preserving strict high performance by skipping unnecessary upcasting conversions in the hot per-pixel inner loop.
