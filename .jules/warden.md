...
## 2025-03-03 - [SIMD Alignment Crash and Uninitialized Memory UB]
**Threat:** [Unaligned Memory Access DoS & Uninitialized Memory UB]
The `rasterize_scanline_simd` function used `_mm256_store_ps` to write to the `depths` buffer. This intrinsic requires strict 32-byte alignment, but `depths` (f32) and `pixels` (u32) can easily have mismatched alignments when sliced. This causes a Segmentation Fault on mismatched alignment boundaries.
Separately, several struct arrays of types `[PreparedTriangle; 8]` and `[MaybeUninit<V>; N]` were being instantiated using `MaybeUninit::uninit().assume_init()`, which is Undefined Behavior in Rust since it tricks the compiler into treating uninitialized bit patterns as valid structs/floats without zeroing them.
**Defense:** [Swapped Intrinsic and Used `zeroed` / Const Arrays]
Swapped `_mm256_store_ps` and `_mm256_store_si256` for their unaligned counterparts: `_mm256_storeu_ps` and `_mm256_storeu_si256`. Replaced `uninit().assume_init()` with `zeroed().assume_init()` for structured arrays to prevent UB, and used `[const { MaybeUninit::uninit() }; N]` for actual `MaybeUninit` arrays.
