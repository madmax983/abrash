...
**2026-04-19 - Memory Exhaustion DoS and Unchecked Slice Indexing**
**Threat:** The `export_ansi` method in `ascii.rs` used unchecked `from_utf8_unchecked` and calculated capacity using a raw multiplication `width * 20 * height` that could overflow `u32` and lead to capacity panic or massive unbounded allocation (DoS). Meanwhile, `fill_circle` in `circle.rs` used `get_unchecked_mut` to write to the framebuffer slice, relying on fast-path bounding box math which could also be bypassed on edge cases (integer overflow wrap-around computing coordinates), leading to out-of-bounds heap memory corruption.
**Defense:** Replaced `from_utf8_unchecked` with safe `from_utf8(&...).unwrap()`. Added `saturating_mul` for capacity logic to prevent multiplication wrap-arounds. Replaced all `get_unchecked_mut` in Bresenham's rasterizer with explicit `slice.get_mut` wrappers wrapping `.wrapping_add()` and `.wrapping_mul()` bounds logic to guarantee array boundary verification.
