...
**2026-03-02 - [Fix Integer Overflow in core::prepare_scanline]**
**Threat:** Integer overflow vulnerability in `src/rasterizer/core.rs` within `prepare_scanline`. When clipping horizontal coordinates, the statement `let diff = -xs as f32;` parses as `-(xs as f32)` conceptually but actually executes `(-xs) as f32`. If an extreme negative coordinate `xs == i32::MIN` is provided, negating it results in an overflow panic in debug mode (DoS) or unexpected wrap-around in release mode, circumventing the intended clipping boundary calculation.
**Defense:** Hardened the calculation by casting the `i32` to `i64` before negation: `let diff = -i64::from(xs) as f32;`, matching the strategy used in other rasterization functions and preventing the boundary bypass.
