...
## 2026-05-18 - [Memory Safety: Texture Dimension Tampering]
**Threat:** The `Texture` struct exposed its fields (`width`, `height`, `width_shift`, `pixels`, etc.) as `pub`. This allowed safe code to arbitrarily mutate `width_shift` or `width` and `height` without changing the backing pixel buffer. A malicious modification could trick safe accessor functions like `get_pixel_texel` into reading wildly out-of-bounds heap memory by evaluating the unsafe fast-path indexing block `*self.pixels.get_unchecked(self.row_offset(y) + x)`.
**Defense:** Made the vulnerable fields private (`width`, `height`, `width_shift`, `pixels`, `mips`, `filter_mode`) and exposed safe public getters and setters. This encapsulates the layout invariants of `Texture`, preventing dimension spoofing from triggering undefined behavior in fast-path slice indexing.
