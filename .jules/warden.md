...
**2023-10-27 - Remove Unsafe Memory Access in Framebuffer and ZBuffer**
**Threat:** Buffer Overflow / Undefined Behavior. The core `Framebuffer` and `ZBuffer` structures exposed `_unchecked` methods that bypassed array bounds checking using `unsafe { get_unchecked_mut }`. If caller logic (such as rasterization span calculation) contained mathematical flaws, this could lead to arbitrary out-of-bounds memory writes, causing silent memory corruption, segmentation faults, or potential code execution vulnerabilities.
**Defense:** Hardened the API boundary by removing the `unsafe` keyword from these methods and replacing raw pointer offsets with standard Rust slice indexing (`self.pixels[idx]`). This ensures that any out-of-bounds access safely panics rather than corrupting memory, adhering to the "Defense in Depth" and "Trust no one" principles. Updated caller sites across the rasterizer to remove unnecessary `unsafe` blocks.
