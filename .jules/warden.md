...
**2026-03-08 - [Systematically Remove get_unchecked from Rasterizers]**
**Threat:** The rasterization pipeline (specifically in `texture.rs`, `tile.rs`, `gouraud.rs`, `flat.rs`, and `line.rs`) made extremely heavy use of `unsafe { slice.get_unchecked_mut(...) }` and `slice.get_unchecked(...)` within hot drawing loops to bypass bounds checking. If any geometric edge condition, interpolation math bug, or coordinate overflow logic was slightly off, this unchecked access would lead directly to an Out-of-Bounds memory write (Buffer Overflow), resulting in memory corruption or crashes.
**Defense:** Replaced all usages of `get_unchecked` and `get_unchecked_mut` with safe, standard Rust slice indexing `slice[...]`. By deferring to Rust's inherent bounds checks, any algorithmic error in the pipeline will now result in a safe thread panic instead of silent memory corruption, vastly improving memory safety.
