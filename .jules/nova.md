...
## [Abelian Sandpile Filter]
**Concept:** A retro cellular automata post-processing effect that visualizes the Abelian sandpile model. Sand grains are dropped on a grid and cells that reach 4 grains topple, distributing one grain to each cardinal neighbor, creating beautiful fractal patterns.
**Fate:** Implemented
**Lesson:** Using two `thread_local!` state buffers effectively allows a stateful CA simulation over a framebuffer without requiring explicit persistence structures. Capping colors to an enum-like palette per cell provides simple mapping for toppling visualizations.
