...
## [Pointillism Filter]
**Concept:** A post-processing effect that simulates a Pointillist painting style by splatting randomized colored dots sampled from the source image.
**Fate:** Implemented
**Lesson:** Using a procedural hash combined with a localized coordinate evaluation approach (painter's algorithm per cell) allows for a purely mathematical, parallelizable splatting technique without requiring actual draw commands or heap-allocating sorting mechanisms.
