...
**Concept:** A post-processing effect that simulates viewing the scene through frosted glass by randomly displacing the sampling coordinate of each pixel within a radius.
**Fate:** Implemented
**Lesson:** When executing random neighbor scatter post-processing effects in parallel with Rayon, clone the source framebuffer to prevent read/write tearing. To maintain deterministic and lock-free execution across parallel chunks, re-seed an inline PRNG (like XorShift32) per-pixel based on its coordinates and an offset seed.
