...
## [Lightning Filter]
**Concept:** A procedural post-processing effect simulating electrical discharges using a recursive fractal branching algorithm. Line segments are perturbed via normal displacement and branched stochastically, drawn with additive blending.
**Fate:** Implemented
**Lesson:** Using simple integer-based additive line drawing combined with recursive midpoint displacement creates convincing lightning very efficiently. Passing the pseudorandom generator mutably down the recursion tree ensures deterministic but chaotic patterns without needing large buffer allocations.
