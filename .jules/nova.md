...
**Concept:** A post-processing effect that approximates an image using hundreds of intersecting straight lines (threads) between pegs arranged in a circle. It uses a greedy algorithm and Bresenham's line rasterization to find and draw lines that cover the "darkest" remaining areas.
**Fate:** Implemented
**Lesson:** Using a thread-local score buffer and greedily evaluating Bresenham line paths allows for an extremely fast simulation of string art generation without needing complex geometric constraint solvers.
