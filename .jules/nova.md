...
## [Strange Attractors Generator]
**Concept:** A procedural rendering tool that generates chaotic strange attractors (like Clifford and Peter de Jong). It iterates non-linear mathematical equations millions of times, tracks the hit density per pixel, and maps the result to the framebuffer logarithmically to produce ethereal, organic structures.
**Fate:** Implemented
**Lesson:** Iterating discrete math functions using raw f32 and calculating hit density logarithmically is extremely fast and yields incredible, infinitely complex organic shapes with a tiny code footprint. It requires tracking density via an intermediate grid before mapping to color for the best visual output.
