...
**Concept:** A post-processing effect that simulates the persistence of vision and the slow decay of phosphors on vintage CRT monitors or early LCD panels. It accumulates frames using a thread-local buffer and blends them using an alpha decay or luminous max operator, creating ghosting trails behind moving objects.
**Fate:** Implemented
**Lesson:** Using a persistent thread-local accumulator buffer is a lightweight and performant way to implement temporal effects like motion blur and ghosting without requiring the main application to pass state back into the rendering pipeline every frame.
