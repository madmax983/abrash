...
**Concept:** A post-processing effect that calculates the luminance histogram and cumulative distribution function (CDF) of the framebuffer, and maps the pixel values to evenly distribute intensities. This acts as an automatic contrast enhancement filter.
**Fate:** Implemented
**Lesson:** Iterating through the framebuffer to build the histogram and mapping array separately avoids complex per-pixel evaluations. Applying the mapping table back to the pixels using parallel iteration allows fast, dependency-free processing.
