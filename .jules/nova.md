...
## [Stateful Motion Blur]
**Concept:** A post-processing effect that simulates camera or object motion blur by maintaining a temporal history buffer of the previous frame, blending the current frame into it using a feedback coefficient.
**Fate:** Implemented
**Lesson:** Encapsulating temporal state inside a dedicated `MotionBlur` struct is superior to using `thread_local!` buffers, as it prevents hidden global state, allows for multiple independent blur instances, and eliminates test flakiness in parallel runners.
