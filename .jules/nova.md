...
## [String Art Filter]
**Concept:** A procedural post-processing effect that approximates an image using a single continuous thread woven between pins on a circle's edge, mimicking a physical string art canvas.
**Fate:** Implemented
**Lesson:** Using a greedy algorithm with Bresenham's line scoring over a target luminance array efficiently estimates the continuous threading process without needing complex linear system solvers. Precomputing pins and tracking line opacity effectively minimizes repeating paths.
