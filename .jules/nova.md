...
## [Pointillism Filter]
**Concept:** A post-processing effect that applies a pointillism filter, using a procedural painter's algorithm with localized per-cell coordinate evaluation to splat randomized overlapping shapes without heap allocations or explicitly ordered draw commands.
**Fate:** Implemented
**Lesson:** Splatting overlapping dots can be highly parallelized by iterating destination pixels and checking distances to neighboring procedural splats, rather than explicitly drawing shapes into a mutable buffer sequentially.
