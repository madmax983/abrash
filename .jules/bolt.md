...
**[Bitwise Comparisons for Floating-Point Equality]**
**Learning:** In hot per-pixel rendering loops or SIMD processing functions (e.g., `apply_heat_vision`), checking for exact floating-point equality against infinity (`depth == f32::INFINITY`) incurs evaluation overhead. Replacing it with an integer bitwise comparison (`depth.to_bits() == 0x7F80_0000`) bypasses floating point unit evaluation. For SIMD processing, `_mm256_castps_si256` can be used alongside `_mm256_cmpeq_epi32` against an integer vector containing the exact bits to bypass the `_mm256_cmp_ps` float comparison completely.
**Action:** Replace `f32::INFINITY` equality checks with bitwise integer comparisons in hot loops to increase performance.
