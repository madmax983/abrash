...
**[Heat Vision Branchless LUT Optimization]**\n**Learning:** In the `apply_heat_vision` scalar hot loop (and SIMD tail loop), we previously had a conditional branch `if depth == f32::INFINITY` that checked for background pixels inside the per-pixel iteration. By slightly adjusting the depth scale (`1023.999 / range`) and expanding the Look-Up Table (LUT) to 1025 elements where `lut[1024]` holds the `f32::INFINITY` background color, we can replace the conditional branch with branchless math `t.min(1024)`.\n**Action:** Replaced conditional branching in `apply_heat_vision` with a 1025-element LUT and branchless scalar math, yielding measurable performance improvements.
