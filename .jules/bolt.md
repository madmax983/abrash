...
**ResourcePool pre-allocation and RayTracing Instance vector capacity**
**Learning:** Always provide and utilize a `with_capacity` constructor for generic resource or object pools (e.g., `ResourcePool`) whose sizing requirements are loosely predictable at instantiation. Pre-allocating entry vectors eliminates multiple heap reallocations when registering initial assets and yields measurable speedups during boot/loading. Also, inside hot per-frame rendering paths (like `encode_rt_shadow_pass`), avoid using `Vec::new()` for instance accumulation when the maximum length (e.g., `prepared_draws.len()`) is known up front.
**Action:** Implemented `ResourcePool::with_capacity` and utilized it inside `CpuRenderer::new` with sensible defaults. Modified `encode_rt_shadow_pass` to pre-allocate its instance vector using `prepared_draws.len()`.
