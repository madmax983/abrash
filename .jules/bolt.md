...
**[Performance Optimization: Eliminate thread_local clone for owned data]**
**Learning:** Using a `thread_local!` buffer to temporarily store transformed vertices in a rendering loop (`Scene::extract`), only to immediately `.clone()` it to satisfy an owned value requirement (e.g., `DrawBatch::new`), is a counterproductive anti-pattern. It incurs the same `Vec` heap allocation per frame while adding a full memory `memcpy` overhead.
**Action:** Remove the `thread_local!` cache entirely for this data path. Instead, allocate the required `Vec::with_capacity()` directly in the loop, write uninitialized data into it via `.spare_capacity_mut()`, and pass the owned `Vec` directly to the constructor to avoid the redundant `.clone()`.
