...
**Optimize SoftBody initialization using foldhash**
**Learning:** `std::collections::HashSet` uses SipHash, which is cryptographically secure but very slow for simple, small data types like integer tuples (edges in a mesh). This becomes a bottleneck during initialization where many elements are inserted.
**Action:** Replaced `std::collections::HashSet` with `foldhash::HashSet` in `SoftBody::new` to avoid SipHash overhead when deduplicating edges. This resulted in a >60% performance improvement in the `new_30x30` benchmark (from ~190µs to ~72µs).
