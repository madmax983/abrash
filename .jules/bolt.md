...
**[Performance Optimization: Eliminating per-frame thread_local allocations in Rayon]**
**Learning:** Using `.for_each_init(|| vec![0u32; len], ...)` inside a Rayon parallel loop does indeed restrict allocations to one per worker thread per loop execution. However, if this Rayon loop is called *every frame* (like in `render_batch`), these thread-local vectors are created and dropped on every single frame, causing a high volume of dynamic heap allocations over time.
**Action:** To achieve true zero-cost abstractions across frames, replace `.for_each_init` with `.for_each` and declare a module-level `thread_local! { static BUFFER: std::cell::RefCell<Vec<T>> = ... }`. When accessing a tuple of vectors inside the `RefCell`, remember to explicitly destructure the borrow (`let (pixels_vec, depths_vec) = &mut *b;`) to avoid multiple mutable borrow errors (`E0499`).
