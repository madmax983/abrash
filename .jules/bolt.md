...
**[Eliminate dynamic allocation in parallel iterators]
**Learning:** When building collections using Rayon's parallel iterators, eliminate intermediate heap allocations caused by `.collect::<Result<Vec<_>, _>>()` by hoisting fallible operations (e.g., resource handle validation) into a preceding sequential loop. This renders the parallel mapping infallible, allowing the use of `.par_extend()` to populate a pre-allocated vector directly.
**Action:** Always check if a `.collect()` inside a performance-critical loop can be refactored to an infallible `.par_extend()` into a pre-allocated vector.
