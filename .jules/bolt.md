...
**[Float to Int Cast Optimization in Hot Loops]**
**Learning:** In hot per-pixel rendering loops (like the `apply_heat_vision` effect scalar loops), casting `f32` directly to `u32` in safe Rust incurs a severe performance penalty due to LLVM injecting saturating conversion instructions.
**Action:** To safely optimize this on x86 without `unsafe`, cast to `i32` first (which maps directly to the fast truncating `cvttss2si` instruction), clamp with `.max(0)`, and then cast to `u32`.
