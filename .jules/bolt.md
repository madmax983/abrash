...
**Eliminating Per-Frame Allocations in Raytracing with SoA**
**Learning:** In the experimental software raytracer (`src/experimental/raytracer.rs`), pre-calculating bounds for all objects per frame using a `Vec<RenderObject>` structure (combining `&SceneObject` + `AABB`) resulted in a dynamic heap allocation every frame because you cannot easily persist structs holding short-lived references (`&'a SceneObject`) in static thread-locals due to lifetime constraints. Also, when using Rayon's `par_chunks_mut`, you cannot borrow a `RefMut` across threads (`!Send`).
**Action:** Instead of an Array-of-Structs (AoS) holding references, use a Structure-of-Arrays (SoA) approach. Maintain a `thread_local!` buffer of purely owned data (`Vec<AABB>`). Inside the frame, clear and populate this buffer, then extract an immutable slice (`let aabbs_slice: &[AABB] = &world_aabbs;`) to freely pass into Rayon closures safely. Inside the hot loop, `.zip()` the `aabbs` slice with the `objects` slice to iterate without dynamic allocations or lifetime gymnastics.
