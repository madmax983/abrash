...
**2024-05-24 - Framebuffer clear_rect chunking optimization**
**Learning:** When optimizing 2D region fills (like `clear_rect`) over a 1D pixel buffer, standard safe iterator-based chunking (`.chunks_exact_mut()`) outperforms explicit index calculations with `unsafe { get_unchecked_mut() }`.
**Action:** Replaced explicit index calculations and `unsafe` blocks with safe `.chunks_exact_mut()` in `Framebuffer::clear_rect` and `ZBuffer::clear_rect`.
