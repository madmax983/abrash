...
## Duplicate Reserve Elimination
**Learning:** Calling `.reserve()` immediately before `.reserve_exact()` on a `Vec` is redundant and causes unnecessary capacity calculation and boundary check overhead.
**Action:** Use only `.reserve_exact()` when the target capacity is explicitly known, and remove any preceding generic `.reserve()` calls.
