...
**[Array Destructuring Extend - Mesh Generation]**
**Learning:** Replacing sequential `.push()` calls within a hot loop (like mesh primitive generation) with a single `.extend([a, b, ...])` call using array destructuring allows the compiler to elide repetitive vector bounds checks, leading to measurable performance improvements.
**Action:** Replaced sequential `indices.push()` calls with `indices.extend([])` in `Mesh::sphere`, `Mesh::plane`, `Mesh::cylinder`, and `Mesh::torus`.
