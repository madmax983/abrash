...
**[Environment Texture Initialization]**
**Learning:** In the environment face pixel format conversion loop, replacing `.extend_from_slice()` with a pre-allocated zeroed vector (`vec![0u8; len * 4]`) and `.chunks_exact_mut(4).zip(pixels.iter())` yields improved performance.
**Action:** Replaced `extend_from_slice()` when performing ARGB to RGBA byte conversion in `crates/abrash-gpu-render/src/environment.rs` with pre-allocation and `chunks_exact_mut` + `zip`.
