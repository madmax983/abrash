...
**Flush and Readback Optimization**
**Learning:** Returning a `Vec` from `flush_and_readback` and doing `data.to_vec()` internally forces a dynamic heap allocation.
**Action:** By changing the function to accept `out: &mut Vec<u8>` and utilizing `out.extend_from_slice()`, we enable the caller to reuse the same vector across frames, saving a large heap allocation per call.
