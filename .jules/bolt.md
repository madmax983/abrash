...
**[Performance Optimization: Eliminate Double-Pass O(N) Iteration for Exact String Capacity]**
**Learning:** While calculating the exact required capacity for a string or vector using an initial O(N) pass avoids intermediate reallocations, the cost of iterating over large data sets twice (once to measure, once to write) can outweigh the benefits of `Vec::with_capacity(exact_len)`. In procedural generation (like L-Systems), this double-pass is a noticeable bottleneck.
**Action:** Replace double-pass exact capacity calculation with double-buffering. Allocate a single reusable buffer (`String` or `Vec`) outside the loop, use `.clear()` and `.reserve(current.len() * 2)` inside the loop, and then `std::mem::swap` at the end of the iteration. Move safety checks (`if len > limit`) inside or directly after the generation loop to maintain security without needing a full pre-pass.
