...
## [f32::total_cmp]
**Learning:** Float comparisons using `partial_cmp().unwrap_or(...)` in `sort_unstable_by` are slow due to branching and NaN checks. Rust's native `f32::total_cmp` achieves the exact same branching optimization and performance gains with drastically simpler syntax.
