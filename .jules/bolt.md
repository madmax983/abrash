...
**[retain_mut vs swap_remove for particle systems]**
**Learning:** In particle systems and similar collections, combining math updates and conditional removals into a single `.retain_mut()` call can be significantly faster (up to ~19% in `ParticleSystem::update`) than manual index-based looping with `swap_remove()`. `.retain_mut()` elides manual bounds checks inside the hot loop and retains contiguous processing.
**Action:** Replace `while` loop and `swap_remove` pattern with `.retain_mut()` when updating and filtering collections in a single pass.
