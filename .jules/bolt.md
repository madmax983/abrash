...
**[Bit-Wise Mapping for Logarithmic Heat Gradients]**
**Learning:** In depth rendering passes (like Heat Vision), replacing inner-loop dynamic float arithmetic calculations (e.g., `(depth - min_z) * scale`) with bit-wise mapping interpolation using `depth.to_bits()` entirely eliminates floating-point arithmetic bottlenecks from the color resolution mapping step.
**Action:** Transformed continuous floating point bounds to integer mapping bounds, yielding a tighter mapping operation without requiring expensive floating point hardware iterations in scalar and SIMD workloads.
