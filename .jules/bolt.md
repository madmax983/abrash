...
## Optimize Float Sorting in Tiled Rendering
**Learning:** Float comparisons using `partial_cmp().unwrap_or(...)` in a `sort_unstable_by` inside rasterizer hot paths are slow due to branching and NaN checks. Instead, mapping IEEE 754 floats to integer space for comparison preserves total ordering and achieves significant performance gains by bypassing floating-point unit (FPU) comparison overhead.
**Action:** Replace `depth_a.partial_cmp(&depth_b).unwrap_or(...)` with a branchless integer-based float mapping (`bits ^ (bits >> 31 & 0x7FFF_FFFF)`) using `key_a.cmp(&key_b)`.
