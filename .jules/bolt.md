...
**[Kahn's algorithm using standard Vec as LIFO stack]**
**Learning:** Kahn's algorithm for topological sorting (e.g., of skeletal joints) does not require a FIFO queue. Replacing `std::collections::VecDeque` with a standard, pre-allocated `Vec` (using `.push()` and `.pop()` as a LIFO stack) eliminates ring-buffer allocation overhead and improves cache locality without changing the algorithm's validity.
**Action:** Replaced `std::collections::VecDeque` with `Vec::with_capacity(n)` in `crates/abrash-skeletal/src/gltf_loader.rs` `topological_sort_joints`.
