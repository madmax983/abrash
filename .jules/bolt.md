...
## [Replace `f32::atan2` with `fast_atan2` in `apply_tunnel`]
**Learning:** In per-pixel post-processing hot loops (like the tunnel effect), using the highly precise standard library `f32::atan2` becomes a CPU bottleneck. Replacing it with a faster approximation (`fast_atan2`) sacrifices imperceptible visual precision for substantial performance gains.
**Action:** Replaced `f32::atan2(dy, dx)` with `fast_atan2(dy, dx)` in `crates/abrash-render/src/experimental/tunnel.rs`.
