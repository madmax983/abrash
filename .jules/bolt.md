...
## [Sepia Filter Fixed-Point Math]
**Learning:** In hot pixel processing loops, replace floating-point arithmetic (e.g., `r * 0.393`) with fixed-point integer math and bit-shifting (e.g., `(r * 402) >> 10`) to eliminate costly float-to-int conversions and floating-point ALU bottlenecks, yielding substantial performance improvements.
**Action:** Implemented the Sepia filter in `crates/abrash-render/src/experimental/sepia.rs` using fixed-point integer math scaled by 1024 to bypass float math completely.
