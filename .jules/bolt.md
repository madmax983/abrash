...
**[Performance Optimization: Declarative Macros for Duplicated Loops]**
**Learning:** When a complex set of nested loops needs to be run in both parallel (`par_chunks_mut`) and scalar (`chunks_mut`) execution paths (e.g., in image filters like Kuwahara), duplicating the logic leads to maintenance issues and potential bugs. However, extracting it into a closure can sometimes inhibit compiler optimizations or cause borrow checker issues with `thread_local!` buffers.
**Action:** Extract the shared inner loop logic into a declarative macro (e.g., `macro_rules! process_row`) that takes the loop bounds, iterators, and state variables as arguments. This ensures identical logic across parallel and scalar paths while maintaining perfect inlining and avoiding closure-related overhead or borrow checker limitations.
