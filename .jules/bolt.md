...
**[Replacing SipHash for Integer Pairs]**
**Learning:** Replacing `std::collections::HashSet` with `foldhash::HashSet` for simple data types like integer pairs yields measurable performance improvements by bypassing the SipHash overhead.
**Action:** Replaced `std::collections::HashSet` with `foldhash::HashSet` in `SoftBody::new`.
