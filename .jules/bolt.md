...
**[Performance Optimization: Thread-local buffers to eliminate to_vec heap allocations in parallel experimental effects]**
**Learning:** Using `fb.as_slice().to_vec()` to clone the source framebuffer (which is necessary to avoid aliasing in non-linear displacement effects like Water Ripple, Swirl, or Chromatic Aberration) causes an expensive memory allocation every frame.
**Action:** Use a `thread_local!` static buffer with `RefCell<Vec<u32>>`. Inside the function, take the buffer out (`buf.take()`), clear it, and extend it from the framebuffer slice (`b.extend_from_slice(fb.as_slice())`). Because the function uses Rayon `par_chunks_exact_mut`, the buffer must be completely extracted to not drop `RefMut` guards across parallel boundaries. At the end of the function, replace the buffer back into the `thread_local!` variable (`buf.replace(src_buffer)`). This provides a zero-cost double buffer and improved benchmark performance by ~10%.
