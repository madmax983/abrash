...
**[Eliding Implicit Heap Reallocations]**
**Learning:** When sequentially inserting elements into a vector inside a loop up to a maximum threshold (e.g., `while vec.len() < max`), omitting a pre-allocation leads to implicit heap reallocations during initialization.
**Action:** Use `vec.reserve_exact(max.saturating_sub(vec.len()))` before the loop to eliminate O(log N) implicit heap reallocations.
