...
**[Eliminating Redundant Vec Capacity Calculations]**\n**Learning:** Calling `.reserve()` immediately before `.reserve_exact()` on a `Vec` causes redundant capacity checking overhead and can occasionally trigger multiple allocations depending on the current capacity boundary relative to the target total capacity, and is considered redundant by performance principles.\n**Action:** Removed redundant `.reserve()` calls when immediately superseded by `.reserve_exact()` to guarantee a single capacity calculation block in hot extraction loops.
