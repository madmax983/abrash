...
**Eliminate Modulo in Wrapping**
**Learning:** Floating point and integer modulo operators (`%`, `.rem_euclid()`) in hot loops are very slow due to division instructions.
**Action:** When wrapping values within a known narrow bounds (like `[-1, 1]`), manual conditional addition/subtraction (`if x < 0 { x += max }`) eliminates the modulo division and results in significantly faster execution.
