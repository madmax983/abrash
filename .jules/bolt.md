...
**[Optimal Vector Capacity Reuse with Extend]**
**Learning:** When reusing a `Vec` across frames to avoid allocations (like `points_out`), calling `out.reserve_exact(len)` immediately after `out.clear()` and right before `out.extend(iter)` actively defeats the vector's capacity-preservation heuristics. The `extend` method, via `FromIterator`, natively checks the iterator's `size_hint()` and handles capacity optimally. If the size of the iterator shrinks compared to the previous frame, a manual `.reserve_exact()` forces a strict reallocation or capacity reduction, whereas simply doing `out.clear(); out.extend(...)` correctly preserves and reuses the existing, larger capacity.
**Action:** Remove `.reserve_exact()` calls before `.extend()` when transforming slice elements into a reused vector buffer.

**[Eliding Bounds Checks on Export Encoding]**
**Learning:** In hot pixel conversion loops (e.g., extracting RGBA channels for PPM/TGA export), building a `Vec` dynamically with `.extend_from_slice()` introduces capacity check overhead on every iteration.
**Action:** Pre-allocate a zeroed vector (`vec![0u8; size]`) and write directly via safe iterators like `.chunks_exact_mut(3)` paired with `.zip()` to allow the compiler to elide bounds checks and significantly improve performance without `unsafe`.
