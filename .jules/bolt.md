...
**[Heat Vision Float to Fixed-Point Bitwise Optimization]**\n**Learning:** When trying to eliminate slow floating-point arithmetic (like `== f32::INFINITY`) in hot rendering loops, replacing it with a direct bitwise integer comparison (`val.to_bits() == 0x7F80_0000`) can yield minor but measurable performance improvements by entirely bypassing the float ALU pipelines.\n**Action:** Replaced `z == f32::INFINITY` with `z.to_bits() == 0x7F80_0000` inside `apply_heat_vision` scalar loops.
