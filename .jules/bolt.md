...
**Plasma Modulo Optimization**
**Learning:** Floating-point modulo operations (`% 1.0`) are extremely slow in hot per-pixel loops because they trigger costly `libm` function calls. When the domain of the input is strictly bounded (e.g., < 2.0 for normalized phase/color wrapping), replacing the modulo with a manual conditional subtraction (e.g., `if c >= 1.0 { c -= 1.0; }`) eliminates the ALU bottleneck and achieves massive performance gains.
**Action:** Replaced `% 1.0` in `plasma.rs` color generation with conditional subtraction, yielding a 25% performance improvement.
