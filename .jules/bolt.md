...
**[Foldhash Optimization in Physics]**
**Learning:** Initializing physics systems (like jelly.rs) often involves finding unique edges using HashSet. The default std::collections::HashSet uses SipHash, which is slow for simple integer pair hashing.
**Action:** Replaced std::collections::HashSet with foldhash::HashSet to significantly reduce overhead and speed up the initialization of physics models.
