...
**Memory: Float comparisons using partial_cmp().unwrap_or(...) in a sort_unstable_by are slow due to branching and NaN checks. Instead, use a branchless integer-based float mapping (e.g., let bits = depth.to_bits() as i32; bits ^ (bits >> 31 & 0x7FFF_FFFF)) and switch to sort_unstable_by_key to mimic IEEE 754 total ordering via integer comparison, eliminating overhead and achieving significant performance gains.**
