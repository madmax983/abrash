...
**[Thermal Vision Float to Fixed-Point Optimization]**
**Learning:** In hot per-pixel rendering loops (like the `apply_thermal_vision` effect), floating-point arithmetic inside conditional branches slows down rendering significantly. Replacing float multiplication and clamps with integer arithmetic by pre-scaling values outside the loop (e.g., mapping `0.0..range` to `0..1023`) completely bypasses the float hardware and yields measurable performance gains. In Rust, floating-point arithmetic is not permitted within `const fn`. When precomputing Look-Up Tables (LUTs) at compile-time to replace hot-loop float calculations, the mapping logic must be completely refactored to use integer arithmetic and bounds checking to compile successfully.
**Action:** Replace floating-point normalization gradients with a pre-computed integer LUT generated via `const fn`.
