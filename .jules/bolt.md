...
**[Parallel clear_rect optimization]**
**Learning:** In parallel 2D region fills over a 1D pixel buffer (like `ZBuffer::clear_rect`), using `.par_chunks_exact_mut()` performs internal chunking overhead that can be optimized. By maintaining the `.par_chunks_exact_mut()` iterator approach but applying `.for_each(|row| row[sx..ex].fill(f32::INFINITY))`, we ensure safe parallelism while avoiding raw pointer casting `self.depths.as_mut_ptr() as usize;`, which violates `Send` constraints. The scalar fallback also utilizes explicit outer loops with `slice.get_unchecked_mut(...)` to bypass runtime bounds checks cleanly.
**Action:** Retained `.par_chunks_exact_mut()` in `ZBuffer::clear_rect` for safe multi-threaded clears while falling back to `unsafe { slice.get_unchecked_mut(...) }` for the scalar loop.
