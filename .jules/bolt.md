...
**[Eliminate dynamic array allocation in metaballs]**
**Learning:** `Vec::with_capacity` paired with pushing elements into vectors inside hot render loops generates a dynamic heap allocation per frame, introducing severe performance overhead and memory fragmentation.
**Action:** Replaced dynamic `Vec::with_capacity` calls with cached `Vec<f32>` fields inside the `Metaballs` struct. Using `.clear()` followed by `.push()` allows the vectors to naturally expand once and reuse their allocated heap capacity on subsequent frames, yielding significant performance gains.
