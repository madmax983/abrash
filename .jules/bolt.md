...
**[Optimize floating-point positive infinity equality checks]**
**Learning:** Checking `depth == f32::INFINITY` can be optimized by using the bitwise check `depth.to_bits() == 0x7F80_0000` to avoid floating point comparison overhead in scalar tail loops.
**Action:** Replace strict floating-point positive infinity checks with the integer bitwise equivalent in hot paths.
