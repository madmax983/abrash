...
**L-System Clone Waste**
**Learning:** In procedural generation tasks (L-Systems), cloning the axiom string unconditionally at the start of `expand` creates a wasted heap allocation if the function uses a fast path (like pure ASCII) that converts the string into a `Vec<u8>` directly from bytes. Furthermore, avoiding a clone for the `iterations == 0` early exit provides a zero-cost abstraction.
**Action:** Ensure clones are only performed exactly when they are needed by moving them into the specific match arms or below early returns, rather than initializing them eagerly at the top of the function.
