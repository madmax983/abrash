...
## [Reduction]
**Bloat:** Abstract `WindowBackend` trait with only conditionally compiled concrete implementations (`Win32Window`, `TuiWindow`). Single-use `Vec2Ext` trait in `sdf.rs`.
**Cut:** Removed `WindowBackend` trait and replaced with inherent `impl` blocks on concrete window types. Removed `Vec2Ext` and replaced `.length()` calls with `.hypot()` directly.
**Saved:** 30 lines of code and removed 2 unnecessary traits.
