...
## [Reduction]
**Bloat:** `src/experimental` folder cluttered with 7 dead modules (`dither`, `glitch`, `isosurface`, `ssao`, `volume`, `voxel_*`) and duplicate `ssao.rs`. Nested `src/gpu/mod.rs` for a single file.
**Cut:** Deleted all dead modules. Flattened `src/gpu/mod.rs` to `src/gpu.rs`. Fixed `src/experimental/raytracer.rs` to use correct `geometry::AABB`.
**Saved:** ~800 lines of dead code, 1 directory level, and removed confusion between `post_process/ssao.rs` and `experimental/ssao.rs`.
