...
## [Reduction]
**Bloat:** Dynamic dispatch (`&mut dyn FnMut`) for closure parameters when static monomorphization is applicable, adding unnecessary runtime overhead.
**Cut:** Refactored `traverse_front_to_back` to accept generic `impl FnMut(usize)` (statically monomorphized as `<F: FnMut(usize)>`), allowing the compiler to optimize the call.
**Saved:** Runtime closure dispatch overhead and eliminated the need for `&mut` at call sites.
