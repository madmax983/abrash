...
## [Reduction]
**Bloat:** `src/experimental` directory with unused modules (`arboretum`, `procedural_mesh`, `voxelizer`) and redundant code (`ssao`). `nova` feature flag used only to hide this dead code.
**Cut:** Deleted `src/experimental` and removed `nova` feature.
**Saved:** 6 files, ~1000 lines of dead/redundant code, and one feature flag.
