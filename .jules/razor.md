...
## [Reduction]
**Bloat:** `Lerp` trait used exclusively in `benches/clipping_optimization.rs` to benchmark a legacy code path.
**Cut:** Deleted the `Lerp` trait and generic `V` type entirely, replacing them with the concrete type `(Vec3, f32)` directly in the function signature and inline math.
**Saved:** Eliminated a redundant trait and localized behavior to the caller, saving lines and maintaining flat abstraction.
