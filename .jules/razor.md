...
## [Reduction]
**Bloat:** The `Lerp` trait and duplicate interpolation functions in `benches/clipping_optimization.rs`.
**Cut:** Deleted the single-use `Lerp` trait entirely and replaced the generic `clip_triangle_to_frustum_legacy` bound with the concrete `(Vec3, f32)` type.
**Saved:** Eliminated a completely unneeded single-implementation trait used only in a benchmark script.
