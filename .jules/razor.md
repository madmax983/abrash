...
## [Reduction]
**Bloat:** The `Lerp` trait and its single `(Vec3, f32)` implementation in `benches/clipping_optimization.rs`.
**Cut:** Deleted the single-use `Lerp` trait entirely and replaced the trait bound `V: Lerp + Copy + Default` in `clip_triangle_to_frustum_legacy` with a simple closure `lerp: impl Fn(V, V, f32) -> V`.
**Saved:** ~20 lines of boilerplate and removed an unnecessary trait abstraction for a benchmark helper.
