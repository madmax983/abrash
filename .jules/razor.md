...
## [Reduction]
**Bloat:** `src/gpu` directory containing only `mod.rs`. Unused experimental modules (`dither`, `glitch`, `isosurface`, `procedural_mesh`, `ssao`, `volume`, `voxel_explosion`, `voxelizer`).
**Cut:** Flattened `src/gpu` to `src/gpu.rs`. Deleted unused modules and `tests/volume_test.rs`.
**Saved:** 8 files, 1 directory level, ~1500 lines of dead code.
