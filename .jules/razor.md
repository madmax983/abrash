...
## [Reduction]
**Bloat:** The single-implementation `Lerp` trait in `benches/clipping_optimization.rs` created specifically for the legacy benchmark.
**Cut:** Flattened the implementation by passing a generic interpolation closure `interpolate: impl Fn(V, V, f32) -> V` to the function instead of relying on a trait.
**Saved:** ~20 lines of code and eliminated an unnecessary trait.
