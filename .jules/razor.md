...
**Bloat:** `SequenceBuilder` and `TweenSegmentBuilder` in `abrash-anim/src/timeline.rs` were entirely unused outside of testing, adding hundreds of lines of "Factory Factory" boilerplate.
**Cut:** Deleted the builders. Users can construct `Timeline`s via `Evaluable` trait objects natively without complex intermediate chained builders.
**Saved:** 2 Structs, ~60 lines of builder code, simplifying the `Timeline` API surface.
