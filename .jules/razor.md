...
## [Reduction]
**Bloat:** Speculative generality in `Sample<T>` storing instantaneous `velocity` which was never used and complicated the `Animatable` trait with math requirements just to compute it.
**Cut:** Removed `velocity` from `Sample<T>`. Removed `anim_scale`, `anim_add`, `anim_sub`, and `zero` from `Animatable`. Simplified `Keyframe` and `Easing` by removing unused analytical derivative tracking.
**Saved:** Dozens of lines of unused code and removed boilerplate requirements for users implementing `Animatable`.
