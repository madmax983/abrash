...
## [Reduction]
**Bloat:** `ChannelTarget` enum that stores duplicate information already captured by the inner types of `ChannelValues` variants.
**Cut:** Removed `ChannelTarget` completely, matching on the variants of `ChannelValues` directly.
**Saved:** Multiple files simplified, avoiding redundant pattern matching and unnecessary intermediate state.
