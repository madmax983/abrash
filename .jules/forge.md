...
**[Winit Event Loop Flattening]**
**Learning:** Winit event loops often lead to deep nesting and repetition ("Pyramid of Doom").
**Action:** Extract the inner event match block into a helper function (e.g., `handle_window_event`) that returns a `Result`, enabling the use of the `?` operator for clean, early returns.
