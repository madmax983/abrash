...
**Array Bounds Checking Deduplication**
**Learning:** When parsing inputs with multiple indices (like OBJ files), inline bounds checks with custom error string formatting result in verbose, repetitive code.
**Action:** Extract repetitive validation logic into a dedicated helper function (e.g., `check_bounds`) to eliminate code duplication and improve readability.
