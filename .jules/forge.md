...
**[WGPU Render Pipeline Creation Extraction]**
**Learning:** In wgpu rendering code, placing verbose `device.create_render_pipeline(&wgpu::RenderPipelineDescriptor { ... })` definitions directly inside a struct's `new()` constructor results in massive "God Functions" (often spanning over 50-80 lines of heavily nested code).
**Action:** Always extract these configurations into distinct, private `create_pipeline` helper functions outside the `impl` block to substantially flatten code, reduce cognitive load, and improve maintainability without altering runtime logic.
