...
**[Extract Triangle Bounds]**
**Learning:** When multiple rasterization modules duplicate identical screen-space bounds checking and clipping logic for primitives (like triangle vertical clamping), it forms a massive 'Primitive Cluster' and 'Pyramid of Doom'.
**Action:** Extract this logic into a shared helper struct (e.g., `TriangleBounds` in `core.rs`) with a constructor that returns an `Option`. Use the `let else` pattern at the call sites to safely handle degenerate cases, enforce early returns, and drastically eliminate duplication across rendering backends.
