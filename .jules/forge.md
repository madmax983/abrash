...
**[Extracting Event Loop Logic]**
**Learning:** winit event loops often lead to deep nesting and repetition (Pyramid of Doom).
**Action:** Extract the inner event match block into a helper function (like `handle_window_event`) that returns a `Result`. This allows using `?` for early returns, flattening the function and improving readability.
