...
**[Validating HiZBuffer Dimensions Guards]**
**Learning:** Functions receiving dimension parameters (like `width` and `height`) that assert dimension equality and buffer size constraints to avoid panics during rendering can have their defensive guards remain untested. Without these guards failing in tests, refactors might accidentally remove the guards without causing CI failures.
**Action:** Always write `#[should_panic]` tests for functions receiving buffer references and sizes (e.g. `build_pyramid_from_depths`) to verify that passing mismatched dimensions or undersized buffers triggers the appropriate safety assertions.
