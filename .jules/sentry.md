...
**[Validating Expect Guards on Allocation Dimensions with Exact Overflows]**\n**Learning:** Relying on  to test expect guards related to allocation dimension limits via  is insufficient because it relies on standard overflow, but it bypasses tighter internal bounds where exact thresholds trigger overflow logic (e.g.  for RGBA mapping).\n**Action:** When testing dimension limit  overflow expect guards, calculate the exact minimum triggering boundary (like ) and explicitly test it to ensure the logic fails predictably right on the edge of the memory bounds, rather than just at arbitrarily massive constants.
**[Validating Expect Guards on Allocation Dimensions with Exact Overflows]**
**Learning:** Relying on `u32::MAX` to test expect guards related to allocation dimension limits via `checked_mul` is insufficient because it relies on standard overflow, but it bypasses tighter internal bounds where exact thresholds trigger overflow logic (e.g. `u32::MAX / 4 + 1` for RGBA mapping).
**Action:** When testing dimension limit `checked_mul` overflow expect guards, calculate the exact minimum triggering boundary (like `(u32::MAX / 4) + 1`) and explicitly test it to ensure the logic fails predictably right on the edge of the memory bounds, rather than just at arbitrarily massive constants.
