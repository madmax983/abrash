...
**Learning:** Even when errors like `StaleHandle` are correctly propagated from internal handle lookup failure via `ok_or()`, there might not be explicit tests verifying the exact enum variant of the returned `Result`. The existing test suite was heavily verifying stale `Mesh` handles, but silently lacked mirror tests for `Material` handles, leaving a logic path untested.
**Action:** Audit functions that interact with multiple types of resource handles (e.g. `Mesh` and `Material`) to ensure that *every* handle type has a corresponding failure test, rather than relying on a single representative test case.
**ZBuffer Clear Rect Extreme Bounds and Hi-Z unwrap_unchecked bounds**
**Learning:** We investigated several potential edge cases that lacked explicit correctness tests under `unsafe` blocks. Specifically, `ZBuffer::clear_rect` handles highly out-of-bounds input using explicit casting to `i64` before doing internal `.get_unchecked_mut` fills. In `TileRenderer`, `hiz_buffer_ref.unwrap_unchecked()` is guarded strictly by an `is_some` flag branch, but this lack of a dedicated correctness test presented a potential coverage gap.
**Action:** Added explicit test loops for `ZBuffer::clear_rect` covering negative bounds, zero bounds, exact capacity limits, and MAX integers (`i32::MAX`/`i32::MIN`/`u32::MAX`). Added a toggling test for the Hi-Z buffer to ensure `unwrap_unchecked()` never panics when flipping the feature. Also added a `to_colored_string_extreme` test for `AsciiConverter` to verify `unwrap()` string handling on hot pixel iteration loops.
