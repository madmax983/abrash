...
**[Missing Guards Tests for Unreachable and Panics]**
**Learning:** Tests can bypass high-level structural constraints inside module tests to test internal assertions (like `unreachable!()` on corrupted `ResourcePool` slots) or expect constraints (`BorrowedRenderTarget::new`) to ensure they panic correctly when memory/state is actually compromised. Also, checking for `.expect` panics during dimension arithmetic on 64-bit systems may only be triggered when constrained via `#[cfg(target_pointer_width = "32")]` if the math does not actually overflow a 64-bit `usize` (like `u32::MAX * u32::MAX`).
**Action:** When adding Sentry coverage for `.expect()` and `unreachable!()` guards, write internal tests to simulate memory/state corruption by directly modifying fields to bypass outer bounds checks and correctly trigger the internal assertions. For math overflows involving `.expect()`, always conditionally verify whether the math can physically overflow the target architecture's pointer width.
