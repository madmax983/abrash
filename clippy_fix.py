...
# Make the structs Copy and Clone by replacing them directly
content = content.replace("pub struct TexSpanState", "#[derive(Clone, Copy)]\npub struct TexSpanState")
content = content.replace("pub struct TexSpanStep", "#[derive(Clone, Copy)]\npub struct TexSpanStep")
content = content.replace("pub struct GouraudSpanState", "#[derive(Clone, Copy)]\npub struct GouraudSpanState")
content = content.replace("pub struct GouraudSpanStep", "#[derive(Clone, Copy)]\npub struct GouraudSpanStep")
content = content.replace("/// Start of a perspective correct texture span.\npub struct PerspectiveSpanStart", "/// Start of a perspective correct texture span.\n#[derive(Clone, Copy)]\npub struct PerspectiveSpanStart")
