...
use crate::rasterizer::{
    ...
    PerspectiveTextureGradients, Texture, is_backface, sort_by_y,
};
...
fn render_triangle_in_tile_textured(
    ...
    for y in y_start..=y_end {
        ...
        if dx <= 0 {
            ...
            let xs = x_start.max(tile_x0).max(0);
            let xe = x_end.min(tile_x1 - 1).min(screen_w - 1);

            if xs <= xe {
                let diff = (xs - x_start) as f32;
                let z_start = z_left + diff * tri.gradients.dz_dx;
                let q_start = q_left + diff * tri.gradients.dq_dx;
                let u_start = u_left + diff * tri.gradients.du_dx;
                let v_start = v_left + diff * tri.gradients.dv_dx;

                let row_offset = ((y - tile_y0) as u32 * TILE_SIZE) as usize;
                let start_col = (xs - tile_x0) as usize;
                let end_col = (xe - tile_x0) as usize;

                let pixels_slice =
                    &mut tile_pixels[row_offset + start_col..=row_offset + end_col];
                let depths_slice =
                    &mut tile_depths[row_offset + start_col..=row_offset + end_col];

                draw_scanline_textured_perspective_tile(
                    pixels_slice,
                    depths_slice,
                    texture,
...
#[inline(always)]
fn draw_scanline_textured_perspective_tile(
    pixels: &mut [u32],
    depths: &mut [f32],
    texture: &Texture,
    ...
    while i < len {
        ...
        let end_idx = (i + count) as usize;
        let p_slice = &mut pixels[i as usize..end_idx];
        let d_slice = &mut depths[i as usize..end_idx];
        ...
        match texture.filter_mode {
            FilterMode::Nearest => {
                ...
                for (pixel, depth_val) in p_slice.iter_mut().zip(d_slice.iter_mut()) {
                    if z < *depth_val {
                        *depth_val = z;
...
impl TileRenderer {
    ...
    pub fn render_batch_textured(
        ...
        for &(v0, uv0, v1, uv1, v2, uv2) in triangles {
            self.prepare_textured_triangle(
                (v0, uv0),
                (v1, uv1),
                (v2, uv2),
                texture.width,
        ...
        // Phase 2: Bin
        self.bin_textured_triangles_cpu();
    ...
    fn bin_textured_triangles_cpu(&mut self) {
    ...
    fn prepare_textured_triangle(
        ...
        for i in 0..clipped.count {
            ...
            let p0_orig = project_to_screen(v0.0.0, v0.0.1, self.width, self.height);
            let p1_orig = project_to_screen(v1.0.0, v1.0.1, self.width, self.height);
            let p2_orig = project_to_screen(v2.0.0, v2.0.1, self.width, self.height);
            ...
            let u0 = v0.1.x * tex_width as f32 * inv_w0;
            let v0_val = v0.1.y * tex_height as f32 * inv_w0;

            let u1 = v1.1.x * tex_width as f32 * inv_w1;
            let v1_val = v1.1.y * tex_height as f32 * inv_w1;

            let u2 = v2.1.x * tex_width as f32 * inv_w2;
            let v2_val = v2.1.y * tex_height as f32 * inv_w2;
