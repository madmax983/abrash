...
/// A clip-space triangle with three vertices `(position, w)` and texture coordinates `uv`.
pub type TexturedClipTriangle = (
    ((Vec3, f32), Vec2),
    ((Vec3, f32), Vec2),
    ((Vec3, f32), Vec2),
);
...
struct PreparedTexturedTriangle {
    ...
    // Vertex attributes (sorted by Y) - needed for edge walking
...
impl TileRenderer {
    ...
    pub fn render_batch_textured(
        ...
        texture: &crate::rasterizer::Texture,
    ) {
        assert_eq!(fb.width(), self.width, "Framebuffer width mismatch");
        assert_eq!(fb.height(), self.height, "Framebuffer height mismatch");
        assert_eq!(zb.width(), self.width, "ZBuffer width mismatch");
        assert_eq!(zb.height(), self.height, "ZBuffer height mismatch");
        ...
        for &(v0, v1, v2) in triangles {
            self.prepare_triangle_textured(v0, v1, v2, texture);
        }
    ...
    fn prepare_triangle_textured(
        ...
        for i in 0..clipped.count {
            ...
            let u0_val = cv0.1.x * texture.width as f32 * inv_w0;
            let v0_val = cv0.1.y * texture.height as f32 * inv_w0;
            let u1_val = cv1.1.x * texture.width as f32 * inv_w1;
            let v1_val = cv1.1.y * texture.height as f32 * inv_w1;
            let u2_val = cv2.1.x * texture.width as f32 * inv_w2;
            let v2_val = cv2.1.y * texture.height as f32 * inv_w2;

            // Sort by Y
            let mut verts = [
                (p0_orig, inv_w0, u0_val, v0_val),
                (p1_orig, inv_w1, u1_val, v1_val),
                (p2_orig, inv_w2, u2_val, v2_val),
            ];
            ...
            // Compute gradients (reuse logic from PerspectiveTextureGradients)
            ...
            let dz_dx = (uy * vz - uz * vy) * inv_nz;
            let dq_dx = (uy * vq - uq * vy) * inv_nz;
            let du_dx = (uy * vu - uu * vy) * inv_nz;
            let dv_dx = (uy * vv - uv * vy) * inv_nz;
...
impl TexturedEdgeWalker {
    fn new(
        ...
        let dx_dy =
            ((i64::from(p_end.x) - i64::from(p_start.x)) as f32 * inv_h * 65536.0) as i64;
...
fn render_triangle_in_tile_textured(
    ...
    let mut edge_a = TexturedEdgeWalker::new(
        tri.p0, tri.p2,
        tri.q0, tri.q2,
        tri.u0, tri.u2,
        tri.v0, tri.v2,
    ...
    let mut edge_b = if y_start < tri.p1.y {
        let mut e = TexturedEdgeWalker::new(
            tri.p0, tri.p1,
            tri.q0, tri.q1,
            tri.u0, tri.u1,
            tri.v0, tri.v1,
    ...
    let use_bilinear = matches!(texture.filter_mode, crate::rasterizer::FilterMode::Bilinear);

    for y in y_start..=y_end {
        if y == tri.p1.y && y != tri.p0.y {
            edge_b = TexturedEdgeWalker::new(
                tri.p1, tri.p2,
                tri.q1, tri.q2,
                tri.u1, tri.u2,
                tri.v1, tri.v2,
        ...
        if dx <= 0 {
            ...
            if x_start >= tile_x0 && x_start < tile_x1 && x_start >= 0 && x_start <= screen_x_max {
                ...
                if z_left < tile_depths[tile_idx] && q_left.abs() > 0.000_001 {
                    ...
                    let u_tex = u_left * w;
                    let v_tex = v_left * w;
                    tile_pixels[tile_idx] = if use_bilinear {
            ...
            if xs <= xe {
                let diff_x = (i64::from(xs) - i64::from(x_start)) as f32;
                let mut z = z_left + diff_x * tri.dz_dx;
                let mut q = q_left + diff_x * tri.dq_dx;
                let mut u = u_left + diff_x * tri.du_dx;
                let mut v = v_left + diff_x * tri.dv_dx;
                ...
                // Span-based perspective correction
                let span_size = 16;
                let mut i = 0;
                let len = pixels.len();
                ...
                while i < len {
                    ...
                    let q_end = q + tri.dq_dx * count as f32;
                    let u_end = u + tri.du_dx * count as f32;
                    let v_end = v + tri.dv_dx * count as f32;
                    ...
                    let pixels_span = &mut pixels[i..i + count];
                    let depths_span = &mut depths[i..i + count];

                    if use_bilinear {
                        for (pixel, depth_val) in
                            pixels_span.iter_mut().zip(depths_span.iter_mut())
                        {
                            if z < *depth_val {
                                *depth_val = z;
...
mod tests {
    ...
    fn textured_triangle_renders_correctly() {
        ...
        tex.set_pixel(1, 1, 0xFF00_FFFF); // Cyan

        // Note: Project to screen will map (0,0) to center.
        // Let's use coordinates that cover the screen.
        // NDC (-1,-1) -> (0,0) uv(0,0) Red
        // NDC (-1, 1) -> (0,h) uv(0,1) Blue
        // NDC ( 1,-1) -> (w,0) uv(1,0) Green

        let v0 = ((Vec3::new(-1.0, 1.0, 1.0), 1.0), crate::math::Vec2::new(0.0, 0.0)); // Top-Left
        let v1 = ((Vec3::new(-1.0, -1.0, 1.0), 1.0), crate::math::Vec2::new(0.0, 1.0)); // Bottom-Left
        let v2 = ((Vec3::new(1.0, 1.0, 1.0), 1.0), crate::math::Vec2::new(1.0, 0.0)); // Top-Right

        tr.render_batch_textured(&mut fb, &mut zb, &[(v0, v1, v2)], &tex);

        // Check top-left pixel (should be Red)
        let tl = fb.get_pixel(0, 0).unwrap();
        // Allow for some interpolation noise, but with Nearest neighbor at 0,0 it should be exact if aligned
        // We are using default filter (Nearest).
        // 0,0 is exactly red.
        assert_eq!(tl, 0xFFFF_0000, "Top-left pixel should be red");
    }
