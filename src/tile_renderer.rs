...
/// A clip-space triangle with three vertices `(position, w)`, UV coordinates, and texture.
    ...
    for y in y_start..=y_end {
        ...
        if dx <= 0 {
            ...
            if xs <= xe {
                ...
                let z_at_xs = z_left + diff * gradients.dz_dx;
                let q_at_xs = q_left + diff * gradients.dq_dx;
                let u_at_xs = u_left + diff * gradients.du_dx;
                let v_at_xs = v_left + diff * gradients.dv_dx;
                ...
                rasterize_scanline_textured_perspective(
                    pixels,
                    depths,
                    texture,
...
#[inline(always)]
#[allow(clippy::too_many_arguments)]
fn rasterize_scanline_textured_perspective(
    pixels: &mut [u32],
    depths: &mut [f32],
    texture: &Texture,
    ...
    while i < len {
        ...
        let pixel_span = &mut pixels[i..i + count];
        let depth_span = &mut depths[i..i + count];
        ...
        match texture.filter_mode {
            FilterMode::Nearest => {
                ...
                for (pixel, depth_val) in pixel_span.iter_mut().zip(depth_span.iter_mut()) {
                    if z < *depth_val {
                        *depth_val = z;
...
impl TileRenderer {
    ...
    /// Render a batch of textured clip-space triangles using the 4-phase tile-based pipeline.
    pub fn render_batch_textured(
        ...
        for &(v0, uv0, v1, uv1, v2, uv2) in triangles {
            self.prepare_textured_triangle(v0, uv0, v1, uv1, v2, uv2, texture);
        }
    ...
    fn prepare_textured_triangle(
        ...
        for i in 0..clipped.count {
            ...
            let gradients = PerspectiveTextureGradients::new(p0, p1, p2, q0, q1, q2, u0, u1, u2, v0, v1, v2);
            let ux = (i64::from(p1.x) - i64::from(p0.x)) as f32;
            let uy = (i64::from(p1.y) - i64::from(p0.y)) as f32;
            let vx = (i64::from(p2.x) - i64::from(p0.x)) as f32;
            let vy = (i64::from(p2.y) - i64::from(p0.y)) as f32;
