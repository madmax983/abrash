...
fn draw_scanline_flat(
    ...
    if xs < 0 {
        ...
        // Prevent panic on i32::MIN negation by casting to f32 first
        z += -(xs as f32) * dz_dx;
        xs = 0;
    }
...
pub fn fill_triangle_3d(
    ...
    for y in y_start..=y_end {
        ...
        // Use f32 for x differences to avoid i32 overflow
        let mut ax = p0.x as f32 + (p2.x as f32 - p0.x as f32) * step.alpha;
        let mut az = p0.z + (p2.z - p0.z) * step.alpha;

        let (mut bx, mut bz) = if step.second_half {
            (
                p1.x as f32 + (p2.x as f32 - p1.x as f32) * step.beta,
                p1.z + (p2.z - p1.z) * step.beta,
            )
        } else {
            (
                p0.x as f32 + (p1.x as f32 - p0.x as f32) * step.beta,
        ...
        // Use i64 to prevent overflow when x_start is i32::MIN and x_end is positive
...
pub fn fill_triangle_gouraud(
    ...
    // Cast to f32 before subtraction to avoid i32 overflow
    let dx_dy_a = (p2.x as f32 - p0.x as f32) * inv_total_height;
    ...
    let x_long_at_p1 = if dy_total != 0 {
        p0.x as f32
            + (p2.x as f32 - p0.x as f32) * ((p1.y - p0.y) as f32 / dy_total as f32)
    } else {
        p0.x as f32
    };
    ...
    let (dx_dy_b1, dz_dy_b1, dc_dy_b1) = if h1 != 0.0 {
        let inv_h1 = 1.0 / h1;
        (
            (p1.x as f32 - p0.x as f32) * inv_h1,
            (p1.z - p0.z) * inv_h1,
            (c1 - c0) * inv_h1,
        )
    } else {
    ...
    if y_start > p0.y {
        ...
        if y_start < p1.y {
            ...
            if h2 != 0.0 {
                let inv_h2 = 1.0 / h2;
                let dx_dy_b2 = (p2.x as f32 - p1.x as f32) * inv_h2;
    ...
    if y_start >= p1.y {
        ...
        if h2 != 0.0 {
            let inv_h2 = 1.0 / h2;
            dx_dy_b = (p2.x as f32 - p1.x as f32) * inv_h2;
    ...
    for y in y_start..=y_end {
        ...
        if xs < 0 {
            // Prevent panic on i32::MIN negation
            let diff = -(xs as f32);
            z += diff * dz_dx;
            c = c + dc_dx * diff;
            xs = 0;
