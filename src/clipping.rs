...
impl<V: Copy> Clone for ClippedTriangles<V> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<V: Copy> Copy for ClippedTriangles<V> {}
...
impl<V> Index<usize> for ClippedTriangles<V> {
    ...
    fn index(&self, index: usize) -> &Self::Output {
        ...
        // SAFETY: Items 0..count*3 are initialized by construction
...
pub fn clip_triangle_to_frustum<V: Lerp + Copy>(
    ...
    if all_in == 0x3F {
        ...
        // Initialize only the first 3 elements
    ...
    // Optimization: Avoid initializing the array
    ...
    macro_rules! clip_plane {
        ($buf_in:ident, $buf_out:ident, $dist_fn:expr) => {
            if count > 0 {
                ...
                // SAFETY: buffer indices are always within 0..count
    ...
    if count >= 3 {
        ...
        for i in 1..count - 1 {
            if result.count < 8 {
                ...
                result.tris[idx + 1].write(unsafe { buf1[i].assume_init() });
                result.tris[idx + 2].write(unsafe { buf1[i + 1].assume_init() });
                result.count += 1;
            }
        }
    }
...
pub fn clip_triangle_against_near_plane<V: Lerp + Copy>(
    ...
    if inside_count == 0 {
        // All outside - cull
        return ClippedTriangles { tris, count: 0 };
    }
    ...
    if out_count == 3 {
        ...
        // Access safely because we know out_count == 4
        let ov0 = unsafe { out_verts[0].assume_init() };
        let ov1 = unsafe { out_verts[1].assume_init() };
        let ov2 = unsafe { out_verts[2].assume_init() };
        let ov3 = unsafe { out_verts[3].assume_init() };

        result.tris[0].write(ov0);
        result.tris[1].write(ov1);
        result.tris[2].write(ov2);

        result.tris[3].write(ov0);
        result.tris[4].write(ov2);
        result.tris[5].write(ov3);
        result.count = 2;
    }
