    ...
    if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
        unsafe {
            draw_scanline_pbr_simd(
                fb, zb, y, x_start, x_end, start, gradients, constants,
            );
        }
        return;
    }
...
unsafe fn draw_scanline_pbr_simd(
    ...
    use std::arch::x86_64::{
        ...
        _mm256_or_si256, _mm256_rsqrt_ps, _mm256_set1_epi32, _mm256_set1_ps, _mm256_set_ps,
    ...
    while remaining >= 8 {
        ...
        if mask_bits != 0 {
            ...
            let n_dot_n = _mm256_fmadd_ps(nx_vec, nx_vec, _mm256_fmadd_ps(ny_vec, ny_vec, _mm256_mul_ps(nz_vec, nz_vec)));
            ...
            let v_dot_v = _mm256_fmadd_ps(vx_raw, vx_raw, _mm256_fmadd_ps(vy_raw, vy_raw, _mm256_mul_ps(vz_raw, vz_raw)));
            ...
            let h_dot_h = _mm256_fmadd_ps(hx_raw, hx_raw, _mm256_fmadd_ps(hy_raw, hy_raw, _mm256_mul_ps(hz_raw, hz_raw)));
            ...
            let n_dot_h_val = _mm256_fmadd_ps(nx, hx, _mm256_fmadd_ps(ny, hy, _mm256_mul_ps(nz, hz)));
            let n_dot_h = _mm256_max_ps(zero, n_dot_h_val);

            // n_dot_v
            let n_dot_v_val = _mm256_fmadd_ps(nx, vx, _mm256_fmadd_ps(ny, vy, _mm256_mul_ps(nz, vz)));
            let n_dot_v = _mm256_max_ps(zero, n_dot_v_val);

            // n_dot_l
            let n_dot_l_val = _mm256_fmadd_ps(nx, lx, _mm256_fmadd_ps(ny, ly, _mm256_mul_ps(nz, lz)));
            let n_dot_l = _mm256_max_ps(zero, n_dot_l_val);

            // h_dot_v
            let h_dot_v_val = _mm256_fmadd_ps(hx, vx, _mm256_fmadd_ps(hy, vy, _mm256_mul_ps(hz, vz)));
            ...
            let ndf_denom = _mm256_mul_ps(pi_vec, _mm256_mul_ps(ndf_denom_term, ndf_denom_term));
            ...
            let ggx2 = _mm256_div_ps(n_dot_v, _mm256_max_ps(ggx_denom_v, _mm256_set1_ps(0.0000001)));
            let ggx1 = _mm256_div_ps(n_dot_l, _mm256_max_ps(ggx_denom_l, _mm256_set1_ps(0.0000001)));
            ...
            let spec_denom = _mm256_fmadd_ps(_mm256_set1_ps(4.0), _mm256_mul_ps(n_dot_v, n_dot_l), epsilon);
            ...
            _mm256_storeu_si256(pixels_ptr as *mut __m256i, _mm256_castps_si256(final_pixels_ps));
        }
    ...
    if remaining > 0 {
        ...
        draw_scanline_pbr_scalar(
            ...
            xs,
            x_end,
            PbrSpanStart { z: z_s, nx: nx_s, ny: ny_s, nz: nz_s, wx: wx_s, wy: wy_s, wz: wz_s },
            gradients,
            constants
        );
    }
...
mod tests {
    ...
    fn test_pbr_simd_matches_scalar() {
        ...
        let gradients = PbrGradients {
            dz_dx: 0.1,
            dnx_dx: 0.01, dny_dx: 0.0, dnz_dx: 0.0,
