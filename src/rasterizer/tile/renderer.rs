use super::context::{AlignedBuffer, SendPtr, TILE_SIZE, TileContext};
use super::types::{
    PreparedGouraudTriangle, PreparedGouraudTrianglesList, PreparedTexturedTriangle,
    PreparedTexturedTrianglesList, PreparedTriangle, PreparedTrianglesList, TileBinIter, TileBins,
};
use super::{render_triangle_in_tile, render_triangle_in_tile_textured};
...
use crate::rasterizer::gouraud::{GouraudEdgeWalker, GouraudGradients};
use crate::rasterizer::texture::{draw_span_bilinear, draw_span_nearest, draw_span_trilinear};
#[cfg(all(feature = "simd", target_arch = "x86_64"))]
use crate::rasterizer::texture::{
    draw_span_bilinear_simd, draw_span_nearest_simd, draw_span_trilinear_simd,
};
use crate::rasterizer::tile::render_triangle_in_tile_gouraud;
...
use super::types::*;
use crate::rasterizer::{ClipTriangle, TexturedClipTriangle};
...
pub struct TileRenderer {
    ...
    pub(crate) prepared_gouraud: Vec<PreparedGouraudTriangle>,
    pub(crate) prepared_textured: Vec<PreparedTexturedTriangle>,
...
impl TileRenderer {
    ...
    pub(crate) fn prepare_triangle_gouraud(
        &mut self,
        v0: ((Vec3, f32), Vec3),
        v1: ((Vec3, f32), Vec3),
        v2: ((Vec3, f32), Vec3),
    ...
    pub(crate) fn prepare_triangle_gouraud_static(
        v0: ((Vec3, f32), Vec3),
        v1: ((Vec3, f32), Vec3),
        v2: ((Vec3, f32), Vec3),
        width: u32,
    ...
    pub(crate) fn bin_triangle_gouraud(&mut self, tri_idx: usize) {
    ...
    #[cfg_attr(feature = "parallel", allow(dead_code))]
    pub(crate) fn prepare_triangle_textured(
        &mut self,
        v0: ((Vec3, f32), Vec2),
        v1: ((Vec3, f32), Vec2),
        v2: ((Vec3, f32), Vec2),
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn prepare_triangle_textured_static(
        v0: ((Vec3, f32), Vec2),
        v1: ((Vec3, f32), Vec2),
        v2: ((Vec3, f32), Vec2),
        tex_w: f32,
    ...
    pub(crate) fn bin_triangle_textured(&mut self, tri_idx: usize) {
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn prepare_triangle_static(
        v0: (Vec3, f32),
        v1: (Vec3, f32),
        v2: (Vec3, f32),
        color: u32,
    ...
    pub(crate) fn bin_triangle(&mut self, tri_idx: usize) {
...
fn render_single_tile_gouraud(
    ...
    let (clear_y_min, clear_y_max): (i32, i32) = clear_tile_bounds(
        tile_bins,
        bin_idx,
        prepared,
        tile_y0,
...
pub(crate) fn process_tile_scanline_gouraud(
    ctx: &mut TileContext,
    y: i32,
    x_start: i32,
    x_end: i32,
...
pub(crate) fn clear_tile_bounds<T>(
    tile_bins: &TileBins,
    bin_idx: usize,
    prepared: &[T],
    tile_y0: i32,
...
pub(crate) fn render_single_tile(
    tx: u32,
    ty: u32,
    tile_bins: &TileBins,
...
pub(crate) fn render_single_tile_textured(
    tx: u32,
    ty: u32,
    tile_bins: &TileBins,
...
pub(crate) fn rasterize_scanline_scalar(
    pixels: &mut [u32],
    depths: &mut [f32],
    z_start: f32,
    dz_dx: f32,
