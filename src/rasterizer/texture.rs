        ...
        for (pixel, depth_val) in fb_slice.iter_mut().zip(zb_slice.iter_mut()) {
            if z < *depth_val {
                ...
                let color = if (u as u32) < tex_w && (v as u32) < tex_h {
                    let idx = ((v as usize) << shift) + (u as usize);
                    let c = tex_pixels[idx];
                    if c == 0xFF000000 {
                         println!("Nearest BLACK: u={} v={} idx={} shift={} len={}", u, v, idx, shift, tex_pixels.len());
                    }
                    c
                } else {
                    let c = texture.get_pixel_texel(u, v);
                    if c == 0xFF000000 {
                         println!("Nearest Fallback BLACK: u={} v={}", u, v);
                    }
                    c
                };
...
/// Fill a projected triangle (screen space) with texture.
#[inline(always)]
#[allow(clippy::too_many_arguments)]
fn fill_projected_triangle_textured(
    ...
    let u0 = uv0.x * texture.width as f32 * inv_w0;
    let v0_val = uv0.y * texture.height as f32 * inv_w0;

    let u1 = uv1.x * texture.width as f32 * inv_w1;
    let v1_val = uv1.y * texture.height as f32 * inv_w1;

    let u2 = uv2.x * texture.width as f32 * inv_w2;
    let v2_val = uv2.y * texture.height as f32 * inv_w2;
...
/// Fill a textured quad using perspective-correct interpolation.
///
/// Efficiently handles quads by projecting all 4 vertices at once (using SIMD)
/// and rasterizing as two triangles. Includes optimization for trivial acceptance (all vertices inside frustum).
///
/// # Arguments
///
/// *   `fb` - Target framebuffer.
/// *   `zb` - Target z-buffer.
/// *   `v0` - Bottom-Left vertex ((Position, W), UV).
/// *   `v1` - Top-Left vertex.
/// *   `v2` - Top-Right vertex.
/// *   `v3` - Bottom-Right vertex.
/// *   `texture` - Texture to map.
pub fn fill_quad_textured(
    ...
    // Fast path: Trivial acceptance check
    // If all vertices have w > near_plane and are within clip bounds, we skip clipping.
    // Near plane for clipping is usually epsilon.
    ...
    if inside(v0.0.0, w0) && inside(v1.0.0, w1) && inside(v2.0.0, w2) && inside(v3.0.0, w3) {
        let width = fb.width();
        let height = fb.height();
        let half_width = width as f32 * 0.5;
        let half_height = height as f32 * 0.5;

        // SIMD Projection of 4 vertices
        ...
        // Rasterize two triangles: 0-1-2 and 0-2-3
        // Assuming Standard Quad Winding: 0=BL, 1=TL, 2=TR, 3=BR
        // Tri 1: BL-TL-TR (0-1-2)
        // Tri 2: BL-TR-BR (0-2-3)

        fill_projected_triangle_textured(fb, zb, p0, p1, p2, v0.1, v1.1, v2.1, texture);
        fill_projected_triangle_textured(fb, zb, p0, p2, p3, v0.1, v2.1, v3.1, texture);
    } else {
        // Fallback: Split into two triangles and use standard clipping pipeline
...
pub fn fill_triangle_textured(
    ...
    for i in 0..clipped.count {
        ...
        fill_projected_triangle_textured(
            fb, zb, p0_orig, p1_orig, p2_orig, v0.1, v1.1, v2.1, texture,
        );
    }
}
