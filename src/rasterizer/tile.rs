...
/// Compact screen point using i16 for coordinates.
///
/// Used to reduce memory bandwidth for binned triangles.
/// Valid range for x/y is [-32768, 32767].
#[derive(Clone, Copy)]
...
impl CompactScreenPoint {
    ...
    pub fn from_screen_point(p: ScreenPoint) -> Self {
        // Clamp to i16 range to avoid wrap-around issues if coordinates are extreme.
        // Although rasterization usually clips to screen, extreme coordinates can happen
        // before clipping or during guard band processing.
        let x = p.x.clamp(i16::MIN as i32, i16::MAX as i32) as i16;
        let y = p.y.clamp(i16::MIN as i32, i16::MAX as i32) as i16;
        Self {
            x,
            y,
            z: p.z,
...
///
/// Optimized layout:
/// - Uses `CompactScreenPoint` (12 bytes) instead of `ScreenPoint` (16 bytes).
/// - Uses `i16` for AABB bounds.
...
///
/// Optimized layout:
/// - Uses `CompactScreenPoint`.
/// - Uses `i16` for AABB.
/// - Total size: ~108 bytes (fits in 2 cache lines).
#[derive(Clone, Copy)]
pub struct PreparedTexturedTriangle {
    pub p0: CompactScreenPoint,
    pub p1: CompactScreenPoint,
    pub p2: CompactScreenPoint,
    pub gradients: PerspectiveTextureGradients, // 28 bytes
    pub u0: f32,
    pub u1: f32,
    pub u2: f32,
    pub v0: f32,
