...
impl TileRenderer {
    ...
    pub fn submit_mesh(&mut self, indices: &[[usize; 3]], vertices: &[(Vec3, f32)], color: u32) {
        #[cfg(feature = "parallel")]
        {
            ...
            // Optimization: `flat_map_iter` streams local iterators directly to the collector,
            // completely avoiding the intermediate per-thread `Vec` heap allocations
            // that occurred previously with `fold(Vec::new, ...).flatten()`.
    ...
    pub fn render_batch(
        ...
        #[cfg(feature = "parallel")]
        {
            ...
            /// Optimization: Avoids intermediate `Vec` allocations per thread partition by
            /// directly streaming items using `flat_map_iter` instead of `fold(Vec::new).flatten()`.
