...
impl Texture {
    ...
    /// # Panics
    ///
    /// Theoretically panics if internal state is inconsistent (missing mips), but this should be unreachable.
    ...
    pub fn get_pixel_trilinear(&self, u: f32, v: f32, lod: f32) -> u32 {
        if lod <= 0.0 || self.mips.is_empty() || lod.is_nan() {
...
mod tests {
    ...
    #[test]
    fn test_trilinear_nan_lod() {
        let mut tex = Texture::new(4, 4).unwrap();
        // Fill texture to make sure sampling works
        for i in 0..16 {
            tex.pixels[i] = 0xFFFFFFFF;
        }
        tex.generate_mipmaps();

        // Should handle NaN gracefully, e.g., by treating it as 0.0 (Bilinear)
        // or at least not panicking.
        let _ = tex.get_pixel_trilinear(0.5, 0.5, f32::NAN);
    }

    #[test]
    fn test_trilinear_inf_lod() {
        let mut tex = Texture::new(4, 4).unwrap();
        for i in 0..16 {
            tex.pixels[i] = 0xFFFFFFFF;
        }
        ...
        // Should handle Infinity (treat as max level)
        let _ = tex.get_pixel_trilinear(0.5, 0.5, f32::INFINITY);
    }
}
