...
impl Texture {
    ...
    /// Load a texture from a PPM file (P3 format).
    ///
    /// # Errors
    ///
    /// Returns an error if the file cannot be read or parsed.
    pub fn load_ppm(path: &Path) -> Result<Self, String> {
        let content = fs::read_to_string(path).map_err(|e| e.to_string())?;

        // Remove comments
        let content_clean: String = content.lines()
            .map(|line| line.split('#').next().unwrap_or(""))
            .collect::<Vec<&str>>()
            .join(" ");

        let mut tokens = content_clean.split_whitespace();

        // Header: P3
        let magic = tokens.next().ok_or("Missing magic number")?;
        if magic != "P3" {
            return Err(format!("Unsupported format: {}", magic));
        }

        // Width, Height
        let width_str = tokens.next().ok_or("Missing width")?;
        let width = width_str.parse::<u32>().map_err(|_| "Invalid width")?;
        let height_str = tokens.next().ok_or("Missing height")?;
        let height = height_str.parse::<u32>().map_err(|_| "Invalid height")?;

        // Max color value
        let max_val_str = tokens.next().ok_or("Missing max value")?;
        let max_val = max_val_str.parse::<u32>().map_err(|_| "Invalid max value")?;

        let mut tex = Self::new(width, height).map_err(|e| e.to_string())?;

        for y in 0..height {
            for x in 0..width {
                let r_str = tokens.next().ok_or("Missing red component")?;
                let g_str = tokens.next().ok_or("Missing green component")?;
                let b_str = tokens.next().ok_or("Missing blue component")?;

                let r = r_str.parse::<u32>().map_err(|_| "Invalid red component")?;
                let g = g_str.parse::<u32>().map_err(|_| "Invalid green component")?;
                let b = b_str.parse::<u32>().map_err(|_| "Invalid blue component")?;

                // Normalize to 0-255
                let r = (r * 255 / max_val).min(255);
                let g = (g * 255 / max_val).min(255);
                let b = (b * 255 / max_val).min(255);
...
/// A sampler wrapper optimized for repeated texture lookups.
/// Caches dimensions to avoid type conversion and offset calculation in hot loops.
#[derive(Clone, Copy)]
...
impl<'a> TextureSampler<'a> {
    ...
    pub fn new(texture: &'a Texture) -> Self {
        Self {
            texture,
            width_m1: texture.width as i32 - 1,
            height_m1: texture.height as i32 - 1,
    ...
    #[must_use]
    pub fn get_pixel_texel(&self, x: i32, y: i32) -> u32 {
        let x = x.clamp(0, self.width_m1) as usize;
        let y = y.clamp(0, self.height_m1) as usize;
        // Use unchecked since we clamped
        unsafe { *self.texture.pixels.get_unchecked(y * self.width + x) }
    }
    ...
    #[inline(always)]
    fn fetch_bilinear_neighbors(&self, x0_raw: i32, y0_raw: i32) -> (u32, u32, u32, u32) {
        // Optimization: Combine bounds check using unsigned cast
        let wm1 = self.width_m1;
        let hm1 = self.height_m1;

        if (x0_raw as u32) < (wm1 as u32) && (y0_raw as u32) < (hm1 as u32) {
            let x0 = x0_raw as usize;
            let y0 = y0_raw as usize;
            ...
            unsafe {
                (
                    *self.texture.pixels.get_unchecked(row0 + x0),
                    *self.texture.pixels.get_unchecked(row0 + x0 + 1),
                    *self.texture.pixels.get_unchecked(row1 + x0),
                    *self.texture.pixels.get_unchecked(row1 + x0 + 1),
                )
            }
        } else {
            // Fallback with clamping
            let x0 = x0_raw.clamp(0, wm1) as usize;
            let y0 = y0_raw.clamp(0, hm1) as usize;
            let x1 = (x0_raw + 1).clamp(0, wm1) as usize;
            let y1 = (y0_raw + 1).clamp(0, hm1) as usize;
            ...
            unsafe {
                (
                    *self.texture.pixels.get_unchecked(row0 + x0),
                    *self.texture.pixels.get_unchecked(row0 + x1),
                    *self.texture.pixels.get_unchecked(row1 + x0),
                    *self.texture.pixels.get_unchecked(row1 + x1),
                )
            }
        }
    }
