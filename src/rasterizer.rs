...
use crate::texture::{FilterMode, Texture, TextureSampler};
...
fn draw_scanline_textured_perspective(
    fb: &mut Framebuffer,
    zb: &mut ZBuffer,
    sampler: TextureSampler, // Changed from texture: &Texture
    y: i32,
    x_start: i32,
    x_end: i32,
    ...
    while x <= xe {
        ...
        match sampler.texture.filter_mode {
            FilterMode::Nearest => {
            ...
            if dx <= 0 {
                    ...
                    && q_left.abs() > 0.000_001
                {
                    ...
                    let color = match texture.filter_mode {
                        ...
                        FilterMode::Bilinear => sampler.get_pixel_bilinear_texel(u_tex, v_tex),
                    };
                    fb.set_pixel(x_start, y, color);
                }
            } else {
