...
fn draw_scanline_gouraud(
    ...
    x_end: i32,
    z_start: f32,
    c_start: (i64, i64, i64, i64), // Fixed point color (R, G, B, A)
    dz_dx: f32,
    ...
    if xs <= xe {
        // Optimization: Use slice iterators to avoid index recalculation and bounds checks
        let width_usize = fb.width() as usize;
        let y_offset = (y as usize) * width_usize;
        let start_idx = y_offset + (xs as usize);
        let end_idx = y_offset + (xe as usize);
        ...
        for (pixel, depth_val) in fb_slice.iter_mut().zip(zb_slice.iter_mut()) {
            let alpha_shifted = a_i.clamp(0, 0x00FF0000);
            let alpha = (alpha_shifted >> 16) as u32;
            ...
            if alpha == 255 {
                ...
                if z < *depth_val {
                    // src components
                    let r_s = (r_i.clamp(0, 0x00FF0000) >> 16) as u32;
                    let g_s = (g_i.clamp(0, 0x00FF0000) >> 16) as u32;
                    let b_s = (b_i.clamp(0, 0x00FF0000) >> 16) as u32;
                    let src_color = (alpha << 24) | (r_s << 16) | (g_s << 8) | b_s;

                    let dst_color = *pixel;

                    // Blend
                    // SWAR optimization
                    // Map alpha 0..255 to 0..256
                    let a = alpha + (alpha >> 7);
                    let inv_a = 256 - a;

                    let rb_src = src_color & 0x00FF00FF;
                    let g_src = src_color & 0x0000FF00;
                    let rb_dst = dst_color & 0x00FF00FF;
                    let g_dst = dst_color & 0x0000FF00;

                    let rb = ((rb_src * a + rb_dst * inv_a) >> 8) & 0x00FF00FF;
                    let g = ((g_src * a + g_dst * inv_a) >> 8) & 0x0000FF00;

                    *pixel = 0xFF000000 | rb | g;
                }
            }
            // else alpha == 0: skip
        ...
        for y in y_start..=y_end {
            ...
            if dx <= 0 {
                if x_start >= 0 && x_start < width_i32 {
                    ...
                    if alpha == 255 {
                        if zb.test_and_set(x_start, y, z_left) {
                            let r = (r_i >> 16).clamp(0, 255) as u32;
                            let g = (g_i >> 16).clamp(0, 255) as u32;
                            let b = (b_i >> 16).clamp(0, 255) as u32;
                            fb.set_pixel(x_start, y, 0xFF000000 | (r << 16) | (g << 8) | b);
                        }
                    } else if alpha > 0 {
                        // Transparent blend path - check Z but don't write
                        if let Some(current_z) = zb.get_depth(x_start, y) {
                            if z_left < current_z {
                                // Need to read pixel from FB to blend
                                if let Some(dst_color) = fb.get_pixel(x_start, y) {
                                    let r_s = (r_i.clamp(0, 0x00FF0000) >> 16) as u32;
                                    let g_s = (g_i.clamp(0, 0x00FF0000) >> 16) as u32;
                                    let b_s = (b_i.clamp(0, 0x00FF0000) >> 16) as u32;
                                    let src_color = (alpha << 24) | (r_s << 16) | (g_s << 8) | b_s;
                                    ...
                                    fb.set_pixel(x_start, y, 0xFF000000 | rb | g);
                                }
                            }
                        }
                    }
...
pub fn fill_triangle_lit(
    ...
    let color_u32 = color_to_u32(Vec4::new(final_color.x, final_color.y, final_color.z, 1.0));
