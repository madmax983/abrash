...
/// Caches width/height and pixel slice for faster access in inner loops.
pub(crate) struct BilinearSampler<'a> {
    pixels: &'a [u32],
    width_i32_minus_1: i32,
    height_i32_minus_1: i32,
    width_usize: usize,
}
...
impl<'a> BilinearSampler<'a> {
    ...
    pub(crate) fn new(texture: &'a Texture) -> Self {
        Self {
            ...
            width_i32_minus_1: texture.width as i32 - 1,
            height_i32_minus_1: texture.height as i32 - 1,
    ...
    fn fetch_bilinear_neighbors(&self, x0_raw: i32, y0_raw: i32) -> (u32, u32, u32, u32) {
        ...
        if x0_raw >= 0
            && x0_raw < self.width_i32_minus_1
            && y0_raw >= 0
            && y0_raw < self.height_i32_minus_1
        {
            ...
            let w_i32 = self.width_i32_minus_1;
            let h_i32 = self.height_i32_minus_1;
            ...
            // SAFETY: We clamped coordinates to valid ranges
            unsafe {
                (
                    *self.pixels.get_unchecked(row0 + x0),
    ...
    #[inline(always)]
    pub(crate) fn get_pixel_bilinear_fixed(&self, u_fixed: i32, v_fixed: i32) -> u32 {
...
impl Texture {
    ...
    pub fn get_pixel_bilinear_fixed(&self, u_fixed: i32, v_fixed: i32) -> u32 {
        BilinearSampler::new(self).get_pixel_bilinear_fixed(u_fixed, v_fixed)
    }
