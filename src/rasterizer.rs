...
impl NormalMapGradients {
    ...
    pub fn new(
        ...
        // Y gradients for UVs (needed for mipmapping, though we might skip for now)
        let ny_q = uq * vx - ux * vq;
        let dq_dy = ny_q * inv_nz;
        let ny_u = uu * vx - ux * vu;
        let du_dy = ny_u * inv_nz;
...
impl NormalMapEdgeWalker {
    ...
    pub fn new(
        ...
        n_start: Vec3,
        n_end: Vec3,
        t_start: crate::math::Vec4,
        t_end: crate::math::Vec4,
    ) -> Self {
        let height = (i64::from(p_end.y) - i64::from(p_start.y)) as f32;
        let inv_h = if height == 0.0 { 0.0 } else { 1.0 / height };
...
fn draw_scanline_normal_mapped(
    ...
    x_start: i32,
    x_end: i32,
    start: PhongSpanStart, // Reuse simpler start struct, we need custom one though?
    // Actually we need NormalMapSpanStart
    start_q: f32,
    start_u: f32,
    start_v: f32,
    start_tx: f32,
    ...
    let mut ty = start_ty;
    let mut tz = start_tz;
    let mut tw = start_tw; // Interpolated handedness
    ...
    for (pixel, depth_val) in fb_slice.iter_mut().zip(zb_slice.iter_mut()) {
        if z < *depth_val {
            *depth_val = z;
            ...
            // Perspective correction
            let w = if q.abs() > 1e-6 { 1.0 / q } else { 1.0 };
            let u_tex = u * w;
            let v_tex = v * w;

            // Base color
            let base_color_u32 = match texture.filter_mode {
            ...
            // Extract base color channels 0.0-1.0
            let base_r = ((base_color_u32 >> 16) & 0xFF) as f32 / 255.0;
            let base_g = ((base_color_u32 >> 8) & 0xFF) as f32 / 255.0;
            let base_b = (base_color_u32 & 0xFF) as f32 / 255.0;
            let base_vec = Vec3::new(base_r, base_g, base_b);

            // Sample Normal Map
            // Normal map stores (x, y, z) mapped to (0..1)
            let nm_val_u32 = match normal_map.filter_mode {
                FilterMode::Nearest => normal_map.get_pixel_texel(u_tex as i32, v_tex as i32),
                FilterMode::Bilinear | FilterMode::Trilinear => {
                    normal_map.get_pixel_bilinear_texel(u_tex, v_tex)
                }
            };

            let nm_r = ((nm_val_u32 >> 16) & 0xFF) as f32 / 255.0;
            let nm_g = ((nm_val_u32 >> 8) & 0xFF) as f32 / 255.0;
            let nm_b = (nm_val_u32 & 0xFF) as f32 / 255.0;
            ...
            // Interpolated basis vectors
            let normal = Vec3::new(nx, ny, nz).fast_normalize();
            let tangent = Vec3::new(tx, ty, tz).fast_normalize();
            // Re-orthogonalize tangent to normal (Graham-Schmidt)
            // T' = T - (N * dot(N, T))
            let tangent = (tangent - normal * normal.dot(tangent)).fast_normalize();

            let bitangent = normal.cross(tangent) * (if tw < 0.0 { -1.0 } else { 1.0 });

            // Construct TBN Matrix (columns are Tangent, Bitangent, Normal)
            // P_world = TBN * P_tangent
            let perturbed_normal = Vec3::new(
                tangent.x * nm_vec.x + bitangent.x * nm_vec.y + normal.x * nm_vec.z,
                tangent.y * nm_vec.x + bitangent.y * nm_vec.y + normal.y * nm_vec.z,
                tangent.z * nm_vec.x + bitangent.z * nm_vec.y + normal.z * nm_vec.z,
            ).fast_normalize();

            // Lighting
            let dot_prod = perturbed_normal.dot(light_dir_neg).max(0.0);
            let diffuse = base_vec * light_color * dot_prod;
            let final_color_vec = base_vec * ambient + diffuse;
...
///
/// Combines texture mapping with per-pixel normal perturbation using a normal map.
#[allow(clippy::too_many_arguments)]
pub fn fill_triangle_normal_mapped(
    fb: &mut Framebuffer,
    zb: &mut ZBuffer,
    v0: ((Vec3, f32), Vec2, Vec3, crate::math::Vec4), // (Pos, W), UV, Normal, Tangent
    v1: ((Vec3, f32), Vec2, Vec3, crate::math::Vec4),
    v2: ((Vec3, f32), Vec2, Vec3, crate::math::Vec4),
    texture: &Texture,
    normal_map: &Texture,
    light_dir: Vec3,
    light_color: Vec3,
    ...
    for i in 0..clipped.count {
        ...
        // Scale UV by texture size for texel coordinates interpolation
        let u0 = v0.1.x * texture.width as f32 * inv_w0;
        let v0_val = v0.1.y * texture.height as f32 * inv_w0;
        let u1 = v1.1.x * texture.width as f32 * inv_w1;
        let v1_val = v1.1.y * texture.height as f32 * inv_w1;
        ...
        sort_by_y(&mut verts, |(p, _, _, _, _)| p.y);
        let [(p0, u0, v0_val, n0, t0), (p1, u1, v1_val, n1, t1), (p2, u2, v2_val, n2, t2)] = verts;
        ...
        let (gradients, long_edge_is_left) = {
            let g = NormalMapGradients::new(
                p0, p1, p2, q0, q1, q2, u0, u1, u2, v0_val, v1_val, v2_val, n0, n1, n2, t0, t1, t2,
            );
            let ux = (i64::from(p1.x) - i64::from(p0.x)) as f32;
            let uy = (i64::from(p1.y) - i64::from(p0.y)) as f32;
            let vx = (i64::from(p2.x) - i64::from(p0.x)) as f32;
        ...
        let mut edge_a = NormalMapEdgeWalker::new(p0, p2, q0, q2, u0, u2, v0_val, v2_val, n0, n2, t0, t2);
        if y_start > p0.y {
            edge_a.step_n(i64::from(y_start) - i64::from(p0.y));
        }

        let mut edge_b = if y_start < p1.y {
            let mut e = NormalMapEdgeWalker::new(p0, p1, q0, q1, u0, u1, v0_val, v1_val, n0, n1, t0, t1);
            if y_start > p0.y {
                e.step_n(i64::from(y_start) - i64::from(p0.y));
            }
            e
        } else {
            let mut e = NormalMapEdgeWalker::new(p1, p2, q1, q2, u1, u2, v1_val, v2_val, n1, n2, t1, t2);
            if y_start > p1.y {
                e.step_n(i64::from(y_start) - i64::from(p1.y));
            }
            e
        ...
        let light_dir_neg = light_dir * -1.0;

        for y in y_start..=y_end {
            if y == p1.y && y != p0.y {
                edge_b = NormalMapEdgeWalker::new(p1, p2, q1, q2, u1, u2, v1_val, v2_val, n1, n2, t1, t2);
            }

            let (x_start, x_end, z_left, q_left, u_left, v_left, nx, ny, nz, tx, ty, tz, tw) = if long_edge_is_left {
                (
                    (edge_a.x >> 16) as i32,
                    (edge_b.x >> 16) as i32,
                    edge_a.z, edge_a.q, edge_a.u, edge_a.v,
                    edge_a.nx, edge_a.ny, edge_a.nz,
                    edge_a.tx, edge_a.ty, edge_a.tz, edge_a.tw,
                )
            } else {
                (
                    (edge_b.x >> 16) as i32,
                    (edge_a.x >> 16) as i32,
                    edge_b.z, edge_b.q, edge_b.u, edge_b.v,
                    edge_b.nx, edge_b.ny, edge_b.nz,
                    edge_b.tx, edge_b.ty, edge_b.tz, edge_b.tw,
                )
            };
            ...
            if dx > 0 {
                draw_scanline_normal_mapped(
                    ...
                    x_start,
                    x_end,
                    PhongSpanStart { z: z_left, nx, ny, nz },
