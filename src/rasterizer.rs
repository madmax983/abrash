...
unsafe fn draw_scanline_gouraud_simd(
    ...
    let dz_dx_vec = _mm256_set1_ps(dz_dx);
    let dr_vec = _mm256_set1_epi32(dc_dx.0);
    let dg_vec = _mm256_set1_epi32(dc_dx.1);
    let db_vec = _mm256_set1_epi32(dc_dx.2);

    let offsets_ps = _mm256_set_ps(7.0, 6.0, 5.0, 4.0, 3.0, 2.0, 1.0, 0.0);
    let offsets_epi32 = _mm256_set_epi32(7, 6, 5, 4, 3, 2, 1, 0);

    let mut z_vec = _mm256_add_ps(_mm256_set1_ps(z_start), _mm256_mul_ps(dz_dx_vec, offsets_ps));

    // r_vec = r_start + dr * offsets
    let mut r_vec = _mm256_add_epi32(
        _mm256_set1_epi32(c_start.0),
        _mm256_mullo_epi32(dr_vec, offsets_epi32),
    );
    let mut g_vec = _mm256_add_epi32(
        _mm256_set1_epi32(c_start.1),
        _mm256_mullo_epi32(dg_vec, offsets_epi32),
    );
    let mut b_vec = _mm256_add_epi32(
        _mm256_set1_epi32(c_start.2),
        _mm256_mullo_epi32(db_vec, offsets_epi32),
    );

    let dz_step = _mm256_mul_ps(dz_dx_vec, _mm256_set1_ps(8.0));
    let dr_step = _mm256_slli_epi32(dr_vec, 3); // * 8
    let dg_step = _mm256_slli_epi32(dg_vec, 3);
    let db_step = _mm256_slli_epi32(db_vec, 3);

    let mask_ff = _mm256_set1_epi32(0x00FF0000); // Mask for 16.16 upper byte (00 RR 00 00)
    ...
    while i + 8 <= len {
        ...
        unsafe {
            ...
            if _mm256_movemask_ps(mask) != 0 {
                ...
                // Mask (remove fractional bits)
                let r_masked = _mm256_and_si256(r_clamped, mask_ff);
                let g_masked = _mm256_and_si256(g_clamped, mask_ff);
                let b_masked = _mm256_and_si256(b_clamped, mask_ff);

                // Pack
                let r_final = r_masked;
                let g_final = _mm256_srli_epi32(g_masked, 8);
                let b_final = _mm256_srli_epi32(b_masked, 16);

                let pixel = _mm256_or_si256(
                    alpha_mask,
                    _mm256_or_si256(r_final, _mm256_or_si256(g_final, b_final)),
                );

                let fb_ptr = fb_slice.as_mut_ptr().add(i) as *mut __m256i;
                let old_color = _mm256_loadu_si256(fb_ptr);
                let new_color = _mm256_blendv_epi8(old_color, pixel, mask_int);
                _mm256_storeu_si256(fb_ptr, new_color);
            }
        }
    ...
    while i < len {
        // SAFETY: Bounds checked by slice length
        unsafe {
            if z < *zb_slice.get_unchecked(i) {
                *zb_slice.get_unchecked_mut(i) = z;
                let r = r_i.clamp(0, 0x00FF0000);
                let g = g_i.clamp(0, 0x00FF0000);
                let b = b_i.clamp(0, 0x00FF0000);

                *fb_slice.get_unchecked_mut(i) = 0xFF000000
                    | ((r as u32) & 0x00FF0000)
