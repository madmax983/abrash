...
use crate::light::{color_to_u32, DirectionalLight};
...
pub fn fill_triangle_lit(
    ...
    let intensity = normal.dot(light.dir * -1.0).max(0.0);
    let d_r = base_color.x * light.color.x * intensity;
    let d_g = base_color.y * light.color.y * intensity;
    let d_b = base_color.z * light.color.z * intensity;
