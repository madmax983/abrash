...
fn draw_scanline_gouraud(
    ...
    if xs <= xe {
        ...
        for (pixel, depth_val) in fb_slice.iter_mut().zip(zb_slice.iter_mut()) {
            // Unpack alpha first
            let alpha = (a_i >> 16).clamp(0, 255) as u32;

            if alpha == 0 {
                // Skip fully transparent pixels (no z-write)
                z += dz_dx;
                r_i += dr;
                g_i += dg;
                b_i += db;
            ...
            if z < *depth_val {
                ...
                if alpha == 255 {
                    ...
                    *pixel = src_color;
                    ...
                    // Transparent pixels DO write to depth buffer (standard behavior for simple transparency)
                    // Wait, memory said: "perform Z-writes for semi-transparent pixels".
                    *depth_val = z;
                    *pixel = blend_colors(src_color, *pixel, alpha);
                }
            }
            z += dz_dx;
            r_i += dr;
...
pub fn fill_triangle_gouraud(
    ...
    for i in 0..clipped.count {
        ...
        for y in y_start..=y_end {
            ...
            if dx <= 0 {
                // Optimization for very short spans (single pixel)
                if x_start >= 0 && x_start < width_i32 {
                     // Check alpha for optimization?
                     let alpha = (c_left.3 >> 16).clamp(0, 255) as u32;
                     if alpha > 0 {
                        // Check depth
                        if zb.test_and_set(x_start, y, z_left) {
                             ...
                             let color = pack_color_channels(r, g, b);
                             if alpha == 255 {
                                fb.set_pixel(x_start, y, color);
                             } else {
                                // For single pixel, we need to read back
                                if let Some(dst) = fb.get_pixel(x_start, y) {
                                    fb.set_pixel(x_start, y, blend_colors(color, dst, alpha));
                                }
                             }
                        }
                     }
