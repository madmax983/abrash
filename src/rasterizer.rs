...
impl PerspectiveTextureGradients {
    /// Calculate Level of Detail (LOD) for trilinear filtering.
    ///
    /// The formula is based on the maximum change of texture coordinates (u, v) per screen pixel.
    ///
    /// # Arguments
    ///
    /// * `q` - Perspective term (1/w) at the pixel.
    /// * `u` - Texture coordinate u/w at the pixel.
    /// * `v` - Texture coordinate v/w at the pixel.
    #[inline(always)]
    #[must_use]
    pub fn calculate_lod(&self, q: f32, u: f32, v: f32) -> f32 {
        let w = if q.abs() > 0.000_001 { 1.0 / q } else { 1.0 };
        let w_sq = w * w;

        // Derivatives of texture coordinates with respect to screen x/y.
        // u_tex = u / q, v_tex = v / q.
        // d(u/q)/dx = (du/dx * q - u * dq/dx) / q^2
        let du_tex_dx = (self.du_dx * q - u * self.dq_dx) * w_sq;
        let dv_tex_dx = (self.dv_dx * q - v * self.dq_dx) * w_sq;
        let du_tex_dy = (self.du_dy * q - u * self.dq_dy) * w_sq;
        let dv_tex_dy = (self.dv_dy * q - v * self.dq_dy) * w_sq;
    ...
    while x <= xe {
        ...
        match texture.filter_mode {
            ...
            FilterMode::Trilinear => {
                ...
                let lod = gradients.calculate_lod(q, u, v);
...
#[inline(always)]
#[allow(clippy::too_many_arguments)]
unsafe fn draw_single_pixel_textured(
    fb: &mut Framebuffer,
    zb: &mut ZBuffer,
    texture: &Texture,
    gradients: &PerspectiveTextureGradients,
    ...
    // SAFETY: Caller ensures x and y are within bounds.
    let z_current = unsafe { zb.get_depth_unchecked(x, y) };
    if z < z_current {
        let w = if q.abs() > 0.000_001 { 1.0 / q } else { 1.0 };
        let u_tex = u * w;
        let v_tex = v * w;
        ...
        if alpha == 255 {
            ...
            unsafe {
                ...
                *zb.as_mut_slice().get_unchecked_mut(idx) = z;
                fb.set_pixel_unchecked(x, y, color);
            }
        } else if alpha > 0 {
            // SAFETY: Caller ensures x and y are within bounds.
            unsafe {
                let dest = fb.get_pixel_unchecked(x, y);
                let blended = blend_swar(color, dest, alpha, 255 - alpha);
