...
pub fn apply_directional_blur(framebuffer: &mut Framebuffer, config: &DirectionalBlurConfig) {
    ...
    // Pre-calculate steps as 16.16 fixed point integers
    let dx_step_fixed = (config.dx * inv_samples * 65536.0) as i32;
    let dy_step_fixed = (config.dy * inv_samples * 65536.0) as i32;

    // Fixed point multiplier for dividing by num_samples (using 16 bits of precision)
    let inv_samples_fixed = (inv_samples * 65536.0) as u32;

    SOURCE_PIXELS.with(|cell| {
        let mut source_pixels = cell.borrow_mut();
