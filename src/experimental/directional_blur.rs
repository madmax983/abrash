...
pub fn apply_directional_blur(framebuffer: &mut Framebuffer, config: &DirectionalBlurConfig) {
    ...
    SOURCE_PIXELS.with(|source_pixels_ref| {
        let mut source_pixels = source_pixels_ref.borrow_mut();
        if source_pixels.len() != framebuffer.as_slice().len() {
            source_pixels.resize(framebuffer.as_slice().len(), 0);
        }
        source_pixels.copy_from_slice(framebuffer.as_slice());
        ...
        let source_slice_ptr = source_pixels.as_ptr() as usize;
        ...
        let process_row = move |(y, row): (usize, &mut [u32])| {
            let source_slice = unsafe { std::slice::from_raw_parts(source_slice_ptr as *const u32, width * height) };
