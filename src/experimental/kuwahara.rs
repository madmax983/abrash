...
macro_rules! process_row {
    ($y_usize:expr, $row:expr, $width:expr, $height:expr, $radius:expr, $src_fb:expr) => {
...
pub fn apply_kuwahara(fb: &mut Framebuffer, radius: i32) {
    ...
    KUWAHARA_BUFFER.with(|buf| {
        ...
        // Only take the slice that matches the framebuffer length instead of resizing entirely
        ...
        #[cfg(feature = "parallel")]
        {
            dest_pixels
                ...
                .for_each(|(y_usize, row)| {
                    process_row!(y_usize, row, width, height, radius, src_fb);
                });
        }
