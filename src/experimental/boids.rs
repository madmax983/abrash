...
impl Flock {
    ...
    ///
    /// ⚡ Bolt Optimization: Uses a reusable `old_boids` buffer with `clone_from`
    /// instead of a dynamic `.clone()` allocation per frame, eliminating
    /// per-frame heap allocations during state snapshotting.
