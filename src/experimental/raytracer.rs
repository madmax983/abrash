...
impl RayTracer {
    ...
    pub fn render(&self, scene: &Scene, fb: &mut Framebuffer) {
        ...
        std::thread_local! {
            static AABBS: std::cell::RefCell<Vec<AABB>> = const { std::cell::RefCell::new(Vec::new()) };
        }

        AABBS.with(|aabbs_cell| {
            let mut aabbs = aabbs_cell.borrow_mut();
            aabbs.clear();
            aabbs.extend(scene.objects.iter().map(crate::scene::SceneObject::calculate_world_aabb));

            // Extract immutable slice to share safely across Rayon threads
