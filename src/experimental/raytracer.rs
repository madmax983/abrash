...
impl RayTracer {
    ...
    ///
    /// ⚡ Bolt Optimization: Eliminates a dynamic heap allocation (`Vec<RenderObject>`)
    /// per frame by pre-calculating world AABBs into a `thread_local!` buffer and
    /// iterating using a Structure-of-Arrays (SoA) approach (`scene.objects.iter().zip(aabbs)`).
    pub fn render(&self, scene: &Scene, fb: &mut Framebuffer) {
        ...
        AABB_BUFFER.with(|buffer| {
            let mut aabbs = buffer.borrow_mut();
            aabbs.clear();
            aabbs.extend(
                scene
                    .objects
    ...
    fn trace_ray(&self, ray: &Ray, objects: &[SceneObject], aabbs: &[AABB], depth: u32) -> u32 {
        ...
        for (obj, aabb) in objects.iter().zip(aabbs) {
