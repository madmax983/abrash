cat << 'INNER_EOF' > crates/abrash-render/src/scene.rs.patch
--- crates/abrash-render/src/scene.rs
+++ crates/abrash-render/src/scene.rs
@@ -270,11 +270,8 @@
...
+            // ⚡ Bolt: Eliminate duplicate `.reserve()` calls before `.reserve_exact()`.
+            draw_list.batches.reserve_exact(visible_count);
+            draw_list.vertices.reserve_exact(total_vertices);

             // ⚡ Bolt: Zip iterator completely avoids bounds checking on cull_results inside the transform loop
INNER_EOF
patch -p0 < crates/abrash-render/src/scene.rs.patch
